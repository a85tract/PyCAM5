  ...
  subroutine compute_cubic_log_native()

    implicit none

    character(len=*), parameter :: proof_msg = 'compute_cubic direct = native'
  ...
  end subroutine compute_cubic_log_native

  logical function compute_cubic_use_native()
    implicit none
    ...
    if (status == 0 .and. n > 0) then
       ...
       compute_cubic_use_native = trim(adjustl(impl_name(:n))) == 'native'
    else
       compute_cubic_use_native = .false.
    end if
  end function compute_cubic_use_native
...
222 continue
 ...
 ! 888   continue 
  ...
  !============================================================================== !
    ...
    if (compute_cubic_use_native()) then
       call compute_cubic_log_native()
       compute_cubic = real(compute_cubic_native_cb(real(a, c_double), real(b, c_double), &
            real(c, c_double)), r8)
       return
    end if
