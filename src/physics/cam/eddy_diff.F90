  ...
  integer,          parameter :: sfdiag_mode_d = 1
  integer,          parameter :: sfdiag_mode_l = 2
  integer,          parameter :: sfdiag_mode_u = 3
  integer,          parameter :: sfdiag_mode_z = 4
  integer,          parameter :: sfdiag_stratus_orig = 0
  integer,          parameter :: sfdiag_stratus_ramp = 1
  integer,          parameter :: sfdiag_stratus_maxi = 2
  ! --------------------------------- !
  ...
  real(r8),         target    :: leng_max(pver) =  40.e3_r8     ! Maximum length scale designed to address issues in the upper
                                                                ! atmosphere.
  ...
  logical                     :: use_native_sfdiag_impl = .false.
  logical                     :: sfdiag_impl_selected = .false.
  ...
  logical                     :: use_native_driver_front_batch_impl = .false.
  logical                     :: driver_front_batch_impl_selected = .false.
  logical                     :: driver_front_batch_entered_logged = .false.
  ...
  ! type(physics_state)     :: state                     ! Physics state variables
    ...
    if (use_native_compute_path) then
       ...
       call eddy_diff_driver_front_batch_call(1, ncol, pcols, pver, 0, kvinit, u, v, t, qv, ql, zero, zero2d, &
            ufd, vfd, tfd, qvfd, qlfd, pmid, taux, tauy, ksrftms, rrho, ustar, minpblh, kvf, kvh_in, kvm_in, &
            kvh_out, kvm_out, kvh, kvm, cpair, latvap, latsub, g, rair, slfd, qtfd, qi, z)
    end if
    ...
    do iturb = 1, nturb
     ...
     ! Compute ustar
       if (use_native_compute_path) then
          ...
          call eddy_diff_driver_front_batch_call(2, ncol, pcols, pver, iturb, kvinit, u, v, t, qv, ql, zero, zero2d, &
               ufd, vfd, tfd, qvfd, qlfd, pmid, taux, tauy, ksrftms, rrho, ustar, minpblh, kvf, kvh_in, kvm_in, &
               kvh_out, kvm_out, kvh, kvm, cpair, latvap, latsub, g, rair, slfd, qtfd, qi, z)
       end if
     ...
     ! This is necessary for 'wstar-based' entrainment closure.
       ...
       if (use_native_compute_path) then
          ...
          call eddy_diff_driver_front_batch_call(3, ncol, pcols, pver, iturb, kvinit, u, v, t, qv, ql, zero, zero2d, &
               ufd, vfd, tfd, qvfd, qlfd, pmid, taux, tauy, ksrftms, rrho, ustar, minpblh, kvf, kvh_in, kvm_in, &
               kvh_out, kvm_out, kvh, kvm, cpair, latvap, latsub, g, rair, slfd, qtfd, qi, z)
       end if
       ...
       if( iturb .lt. nturb ) then
         ...
         if ( waccmx_is('ionosphere') .or. waccmx_is('neutral') ) then 
           ...
           if (use_native_compute_path) then
              ...
              call eddy_diff_driver_tail_batch_call(6, ncol, pcols, pver, ncvmax, lambda, kvh, kvh_out, errorPBL, kvm, &
                   kvm_out, cgh, cgs, sl, qt, u, v, slfd, qtfd, ufd, vfd, ipbl, wstar, wstarPBL, rair_local=rair, &
                   rairi_local=rairi)
           end if
         endif
         ...
         ! use 'trbintd' at the next iteration.
          ...
          if (use_native_compute_path) then
             ...
             call eddy_diff_driver_front_batch_call(4, ncol, pcols, pver, iturb, kvinit, u, v, t, qv, ql, zero, zero2d, &
                  ufd, vfd, tfd, qvfd, qlfd, pmid, taux, tauy, ksrftms, rrho, ustar, minpblh, kvf, kvh_in, kvm_in, &
                  kvh_out, kvm_out, kvh, kvm, cpair, latvap, latsub, g, rair, slfd, qtfd, qi, z)
          end if
       endif
    ...
    if (use_native_compute_path) then
       ...
       call eddy_diff_driver_tail_batch_call(7, ncol, pcols, pver, ncvmax, lambda, kvh, kvh_out, errorPBL, kvm, &
            kvm_out, cgh, cgs, sl, qt, u, v, slfd, qtfd, ufd, vfd, ipbl, wstar, wstarPBL, kvq_local=kvq)
    end if
  ...
  subroutine eddy_diff_append_impl_trace(message)
    ...
    integer :: trace_unit, status, n

    call get_environment_variable('EDDY_DIFF_IMPL_TRACE_FILE', value=trace_file, length=n, status=status)
    if (status /= 0 .or. n <= 0) return

    open(newunit=trace_unit, file=trim(adjustl(trace_file(:n))), status='unknown', action='write', position='append')
    write(trace_unit,'(A)') trim(message)
    close(trace_unit)

  end subroutine eddy_diff_append_impl_trace

  !=============================================================================== !
  !                                                                                !
  !=============================================================================== !

  subroutine eddy_diff_trbintd_core_batch_append_proof(proof_line)
    ...
    proof_file = ''
    call get_environment_variable('EDDY_DIFF_TRBINTD_CORE_BATCH_PROOF_FILE', value=proof_file, length=n, status=status)
    if (status == 0 .and. n > 0) then
  ...
  end subroutine eddy_diff_trbintd_core_batch_append_proof

  !=============================================================================== !
  !                                                                                !
  !=============================================================================== !

  subroutine eddy_diff_trbintd_core_batch_select_impl()
    ...
    if (status == 0 .and. n > 0) then
       ...
       use_native_trbintd_core_batch_impl = trim(adjustl(impl_name(:n))) == 'native'
    else
       use_native_trbintd_core_batch_impl = .false.
    end if

    trbintd_core_batch_impl_selected = .true.

    if (masterproc) then
       if (use_native_trbintd_core_batch_impl) then
          write(iulog,*) 'eddy_diff_trbintd_core_batch implementation = native'
          write(*,*) 'eddy_diff_trbintd_core_batch implementation = native'
          call eddy_diff_append_impl_trace('eddy_diff_trbintd_core_batch implementation = native')
          call eddy_diff_trbintd_core_batch_append_proof('eddy_diff_trbintd_core_batch selector entered implementation = native')
       else
          write(iulog,*) 'eddy_diff_trbintd_core_batch implementation = codon'
          write(*,*) 'eddy_diff_trbintd_core_batch implementation = codon'
          call eddy_diff_append_impl_trace('eddy_diff_trbintd_core_batch implementation = codon')
          call eddy_diff_trbintd_core_batch_append_proof('eddy_diff_trbintd_core_batch selector entered implementation = codon')
       end if
       call flush(iulog)
    end if

  end subroutine eddy_diff_trbintd_core_batch_select_impl

  !=============================================================================== !
  !                                                                                !
  !=============================================================================== !

  subroutine eddy_diff_trbintd_core_batch_log_entered()

    implicit none

    if (trbintd_core_batch_entered_logged) return
  ...
  subroutine eddy_diff_driver_tail_batch_call(stage, ncol, pcols, pver, ncvmax_local, lambda_local, kvh_local, &
       ...
       v_local, slfd_local, qtfd_local, ufd_local, vfd_local, ipbl_local, wstar_local, wstarPBL_local, rair_local, &
       rairi_local, kvq_local)
    ...
    real(r8), optional, intent(in) :: rair_local
    ...
    real(r8), target, optional, intent(inout) :: rairi_local(pcols,pver+1), kvq_local(pcols,pver+1)
    real(c_double) :: rair_c
    type(c_ptr) :: rairi_p, kvq_p

    interface
       subroutine eddy_diff_driver_tail_batch_codon(stage_c, ncol_c, pcols_c, pver_c, ncvmax_c, lambda_c, rair_c, kvh_p, &
            kvh_out_p, errorPBL_p, kvm_p, kvm_out_p, cgh_p, cgs_p, sl_p, qt_p, u_p, v_p, slfd_p, qtfd_p, ufd_p, &
            vfd_p, ipbl_p, wstar_p, wstarPBL_p, rairi_p, kvq_p) bind(c, name="eddy_diff_driver_tail_batch_codon")
         use iso_c_binding, only: c_double, c_int64_t, c_ptr
         integer(c_int64_t), value :: stage_c, ncol_c, pcols_c, pver_c, ncvmax_c
         real(c_double), value :: lambda_c, rair_c
         ...
         type(c_ptr), value :: rairi_p, kvq_p
    ...
    if (use_native_driver_tail_batch_impl) then
       ...
       case (6)
          if (present(rair_local) .and. present(rairi_local)) rairi_local(:ncol,:pver+1) = rair_local
       case (7)
          if (present(kvq_local)) kvq_local(:ncol,:) = kvh_out_local(:ncol,:)
       end select
       return
    end if

    rair_c = 0.0_c_double
    if (present(rair_local)) rair_c = real(rair_local, c_double)
    rairi_p = c_null_ptr
    if (present(rairi_local)) rairi_p = c_loc(rairi_local)
    kvq_p = c_null_ptr
    if (present(kvq_local)) kvq_p = c_loc(kvq_local)
    ...
    call eddy_diff_driver_tail_batch_codon( &
         ...
         int(ncvmax_local, c_int64_t), real(lambda_local, c_double), rair_c, c_loc(kvh_local), c_loc(kvh_out_local), &
         ...
         c_loc(ufd_local), c_loc(vfd_local), c_loc(ipbl_local), c_loc(wstar_local), c_loc(wstarPBL_local), rairi_p, kvq_p)
  ...
  subroutine eddy_diff_driver_front_batch_append_proof(proof_line)
    ...
    proof_file = ''
    call get_environment_variable('EDDY_DIFF_DRIVER_FRONT_BATCH_PROOF_FILE', value=proof_file, length=n, status=status)
    if (status == 0 .and. n > 0) then
       open(newunit=unitno, file=trim(proof_file(:n)), status='unknown', position='append', action='write')
       write(unitno,'(A)') trim(proof_line)
       close(unitno)
  ...
  end subroutine eddy_diff_driver_front_batch_append_proof

  !=============================================================================== !
  !                                                                                !
  !=============================================================================== !

  subroutine eddy_diff_driver_front_batch_select_impl()
    ...
    if (status == 0 .and. n > 0) then
       ...
       use_native_driver_front_batch_impl = trim(adjustl(impl_name(:n))) == 'native'
    else
       use_native_driver_front_batch_impl = .false.
    end if

    driver_front_batch_impl_selected = .true.

    if (masterproc) then
       if (use_native_driver_front_batch_impl) then
          write(iulog,*) 'eddy_diff_driver_front_batch implementation = native'
          write(*,*) 'eddy_diff_driver_front_batch implementation = native'
          call eddy_diff_append_impl_trace('eddy_diff_driver_front_batch implementation = native')
          call eddy_diff_driver_front_batch_append_proof('eddy_diff_driver_front_batch selector entered implementation = native')
       else
          write(iulog,*) 'eddy_diff_driver_front_batch implementation = codon'
          write(*,*) 'eddy_diff_driver_front_batch implementation = codon'
          call eddy_diff_append_impl_trace('eddy_diff_driver_front_batch implementation = codon')
          call eddy_diff_driver_front_batch_append_proof('eddy_diff_driver_front_batch selector entered implementation = codon')
       end if
       call flush(iulog)
    end if

  end subroutine eddy_diff_driver_front_batch_select_impl

  !=============================================================================== !
  !                                                                                !
  !=============================================================================== !

  subroutine eddy_diff_driver_front_batch_log_entered()
  ...
  end subroutine eddy_diff_driver_front_batch_log_entered

  !=============================================================================== !
  !                                                                                !
  !=============================================================================== !

  subroutine eddy_diff_driver_front_batch_call(stage, ncol, pcols, pver, iturb, kvinit_local, u_local, v_local, &
       t_local, qv_local, ql_local, zero_local, zero2d_local, ufd_local, vfd_local, tfd_local, qvfd_local, &
       qlfd_local, pmid_local, taux_local, tauy_local, ksrftms_local, rrho_local, ustar_local, minpblh_local, &
       kvf_local, kvh_in_local, kvm_in_local, kvh_out_local, kvm_out_local, kvh_local, kvm_local, cpair_local, &
       latvap_local, latsub_local, g_local, rair_local, slfd_local, qtfd_local, qi_local, z_local)

    use iso_c_binding, only: c_double, c_int64_t, c_loc, c_ptr

    implicit none

    integer, intent(in) :: stage, ncol, pcols, pver, iturb
    ...
    real(r8), target, intent(in) :: u_local(pcols,pver), v_local(pcols,pver), t_local(pcols,pver), qv_local(pcols,pver), &
         ql_local(pcols,pver), pmid_local(pcols,pver), taux_local(pcols), tauy_local(pcols), ksrftms_local(pcols)
    ...
    interface
       subroutine eddy_diff_driver_front_batch_codon(stage_c, ncol_c, pcols_c, pver_c, iturb_c, kvinit_c, use_kvf_c, &
            rair_c, ustar_min_c, cpair_c, latvap_c, latsub_c, g_c, u_p, v_p, t_p, qv_p, ql_p, zero_p, zero2d_p, &
            ufd_p, vfd_p, tfd_p, qvfd_p, qlfd_p, pmid_p, taux_p, tauy_p, ksrftms_p, rrho_p, ustar_p, minpblh_p, &
            kvf_p, kvh_in_p, kvm_in_p, kvh_out_p, kvm_out_p, kvh_p, kvm_p, slfd_p, qtfd_p, qi_p, z_p) &
            bind(c, name="eddy_diff_driver_front_batch_codon")
         use iso_c_binding, only: c_double, c_int64_t, c_ptr
         integer(c_int64_t), value :: stage_c, ncol_c, pcols_c, pver_c, iturb_c, kvinit_c, use_kvf_c
         real(c_double), value :: rair_c, ustar_min_c, cpair_c, latvap_c, latsub_c, g_c
         type(c_ptr), value :: u_p, v_p, t_p, qv_p, ql_p, zero_p, zero2d_p, ufd_p, vfd_p, tfd_p, qvfd_p, qlfd_p
         type(c_ptr), value :: pmid_p, taux_p, tauy_p, ksrftms_p, rrho_p, ustar_p, minpblh_p
         type(c_ptr), value :: kvf_p, kvh_in_p, kvm_in_p, kvh_out_p, kvm_out_p, kvh_p, kvm_p, slfd_p, qtfd_p, qi_p, z_p
       end subroutine eddy_diff_driver_front_batch_codon
    end interface

    call eddy_diff_driver_front_batch_select_impl()

    if (use_native_driver_front_batch_impl) then
       select case (stage)
       case (1)
          call eddy_diff_init_fields(ncol, pcols, pver, u_local, v_local, t_local, qv_local, ql_local, zero_local, &
               zero2d_local, ufd_local, vfd_local, tfd_local, qvfd_local, qlfd_local)
       case (2)
          call eddy_diff_surface_stress_diag(ncol, pcols, pver, tfd_local, pmid_local, taux_local, tauy_local, &
               ksrftms_local, ufd_local, vfd_local, rrho_local, ustar_local, minpblh_local)
       case (3)
          call eddy_diff_kv_init(ncol, pcols, pver, iturb, kvinit_local, kvf_local, kvh_in_local, kvm_in_local, &
               kvh_out_local, kvm_out_local, kvh_local, kvm_local)
       case (4)
          call eddy_diff_rebuild_thermo(ncol, pcols, pver, cpair_local, latvap_local, latsub_local, g_local, rair_local, &
               slfd_local, qtfd_local, qi_local, z_local, pmid_local, qlfd_local, qvfd_local, tfd_local)
       end select
       return
    end if

    call eddy_diff_driver_front_batch_log_entered()
    call eddy_diff_driver_front_batch_codon( &
         int(stage, c_int64_t), int(ncol, c_int64_t), int(pcols, c_int64_t), int(pver, c_int64_t), int(iturb, c_int64_t), &
         int(merge(1, 0, kvinit_local), c_int64_t), int(merge(1, 0, use_kvf), c_int64_t), real(rair_local, c_double), &
         real(0.01_r8, c_double), real(cpair_local, c_double), real(latvap_local, c_double), real(latsub_local, c_double), &
         real(g_local, c_double), c_loc(u_local), c_loc(v_local), c_loc(t_local), c_loc(qv_local), c_loc(ql_local), &
         c_loc(zero_local), c_loc(zero2d_local), c_loc(ufd_local), c_loc(vfd_local), c_loc(tfd_local), c_loc(qvfd_local), &
         c_loc(qlfd_local), c_loc(pmid_local), c_loc(taux_local), c_loc(tauy_local), c_loc(ksrftms_local), &
         c_loc(rrho_local), c_loc(ustar_local), c_loc(minpblh_local), c_loc(kvf_local), c_loc(kvh_in_local), &
         c_loc(kvm_in_local), c_loc(kvh_out_local), c_loc(kvm_out_local), c_loc(kvh_local), c_loc(kvm_local), &
  ...
  subroutine sfdiag_select_impl()
    ...
    if (status == 0 .and. n > 0) then
       ...
       use_native_sfdiag_impl = trim(adjustl(impl_name(:n))) == 'native'
    else
       use_native_sfdiag_impl = .false.
    end if

    sfdiag_impl_selected = .true.

    if (masterproc) then
       if (use_native_sfdiag_impl) then
          write(iulog,*) 'sfdiag implementation = native'
          write(*,*) 'sfdiag implementation = native'
          call eddy_diff_append_impl_trace('sfdiag implementation = native')
       else
          write(iulog,*) 'sfdiag implementation = codon'
          write(*,*) 'sfdiag implementation = codon'
          call eddy_diff_append_impl_trace('sfdiag implementation = codon')
       end if
    end if

  end subroutine sfdiag_select_impl

  !=============================================================================== !
  !                                                                                !
  !=============================================================================== !

  integer function sfdiag_resolve_mode() result(mode_local)

    implicit none

    select case (sftype)
    case ('d')
       mode_local = sfdiag_mode_d
    case ('l')
       mode_local = sfdiag_mode_l
    case ('u')
       mode_local = sfdiag_mode_u
    case ('z')
       mode_local = sfdiag_mode_z
    case default
       call endrun('eddy_diff: unsupported sfdiag sftype')
    end select

  end function sfdiag_resolve_mode

  !=============================================================================== !
  !                                                                                !
  !=============================================================================== !

  integer function sfdiag_resolve_stratus_mode() result(stratus_mode_local)

    implicit none

    if (choice_evhc .eq. 'ramp' .or. choice_radf .eq. 'ramp') then
       stratus_mode_local = sfdiag_stratus_ramp
    elseif (choice_evhc .eq. 'maxi' .or. choice_radf .eq. 'maxi') then
       stratus_mode_local = sfdiag_stratus_maxi
    else
       stratus_mode_local = sfdiag_stratus_orig
    end if

  end function sfdiag_resolve_stratus_mode
  ...
  subroutine sfdiag( pcols   , pver    , ncol    , qt      , ql      , sl      , &
    ...
    real(r8), target, intent(in)  :: sl(pcols,pver)
    real(r8), target, intent(in)  :: qt(pcols,pver)
    real(r8), target, intent(in)  :: ql(pcols,pver)
    real(r8), target, intent(in)  :: pi(pcols,pver+1)
    real(r8), target, intent(in)  :: pm(pcols,pver)
    real(r8), target, intent(in)  :: zi(pcols,pver+1)
    real(r8), target, intent(in)  :: cld(pcols,pver)
    real(r8), target, intent(in)  :: slslope(pcols,pver)
    real(r8), target, intent(in)  :: qtslope(pcols,pver)
    ...
    mode_local = sfdiag_resolve_mode()
    stratus_mode_local = sfdiag_resolve_stratus_mode()

    if (use_native_sfdiag_impl) then
       call sfdiag_native(mode_local, stratus_mode_local, pcols, pver, ncol, qt, ql, sl, pi, pm, zi, cld, sfi, sfuh, sflh, &
            slslope, qtslope)
       return
    end if
  ...
  subroutine sfdiag_native(mode_local, stratus_mode_local, pcols, pver, ncol, qt, ql, sl, pi, pm, zi, cld, sfi, sfuh, sflh, &
       slslope, qtslope)

    implicit none

    integer, intent(in) :: mode_local, stratus_mode_local, pcols, pver, ncol
    real(r8), intent(in) :: qt(pcols,pver), ql(pcols,pver), sl(pcols,pver), pi(pcols,pver+1), pm(pcols,pver), zi(pcols,pver+1)
    real(r8), intent(in) :: cld(pcols,pver), slslope(pcols,pver), qtslope(pcols,pver)
    real(r8), intent(out) :: sfi(pcols,pver+1), sfuh(pcols,pver), sflh(pcols,pver)

    call sfdiag_driver(mode_local, stratus_mode_local, ntop_turb, nbot_turb, pcols, pver, ncol, qmin, cpair, g, qt, ql, sl, pi, &
         pm, zi, cld, sfi, sfuh, sflh, slslope, qtslope)

  end subroutine sfdiag_native

  !=============================================================================== !
  !                                                                                !
  !=============================================================================== !

  subroutine sfdiag_driver(mode_local, stratus_mode_local, ntop_turb_local, nbot_turb_local, pcols, pver, ncol, qmin_local, &
       cpair_local, g_local, qt, ql, sl, pi, pm, zi, cld, sfi, sfuh, sflh, slslope, qtslope)

    implicit none

    integer, intent(in) :: mode_local, stratus_mode_local, ntop_turb_local, nbot_turb_local, pcols, pver, ncol
    real(r8), intent(in) :: qmin_local, cpair_local, g_local
    real(r8), intent(in) :: qt(pcols,pver), ql(pcols,pver), sl(pcols,pver), pi(pcols,pver+1), pm(pcols,pver), zi(pcols,pver+1)
    real(r8), intent(in) :: cld(pcols,pver), slslope(pcols,pver), qtslope(pcols,pver)
    real(r8), intent(out) :: sfi(pcols,pver+1), sfuh(pcols,pver), sflh(pcols,pver)

    integer  :: i, k, km1
    real(r8) :: sltop, slbot, qttop, qtbot, tltop, tlbot, qxtop, qxbot, qxm, es, qs
    real(r8) :: cldeff_local(pcols,pver)

    sfi(1:ncol,:)    = 0._r8
    sfuh(1:ncol,:)   = 0._r8
    sflh(1:ncol,:)   = 0._r8
    cldeff_local(1:ncol,:) = 0._r8

    select case (mode_local)
    case (sfdiag_mode_d)
       do k = ntop_turb_local + 1, nbot_turb_local
          km1 = k - 1
          do i = 1, ncol
             sfuh(i,k) = cld(i,k)
             sflh(i,k) = cld(i,k)
    ...
    case (sfdiag_mode_l)
       do k = ntop_turb_local + 1, nbot_turb_local
          km1 = k - 1
          do i = 1, ncol
             cldeff_local(i,k) = cld(i,k)
             sfuh(i,k) = cld(i,k)
             sflh(i,k) = cld(i,k)
             if( ql(i,k) .lt. qmin_local ) then
                sfuh(i,k) = 0._r8
                sflh(i,k) = 0._r8
             end if
             if( stratus_mode_local .eq. sfdiag_stratus_ramp ) then
                cldeff_local(i,k) = cld(i,k) * min( ql(i,k) / qmin_local, 1._r8 )
                sfuh(i,k) = cldeff_local(i,k)
                sflh(i,k) = cldeff_local(i,k)
             elseif( stratus_mode_local .eq. sfdiag_stratus_maxi ) then
                cldeff_local(i,k) = cld(i,k)
                sfuh(i,k) = cldeff_local(i,k)
                sflh(i,k) = cldeff_local(i,k)
             end if
    ...
    case default
       call endrun('eddy_diff: unsupported sfdiag mode')
    end select

  end subroutine sfdiag_driver

  !=============================================================================== !
  !                                                                                !
  !=============================================================================== !

  subroutine eddy_diff_sfdiag_native_test_c(mode_c, stratus_mode_c, ntop_turb_c, nbot_turb_c, ncol_c, pcols_c, pver_c, qmin_c, &
       cpair_c, g_c, qt_p, ql_p, sl_p, pi_p, pm_p, zi_p, cld_p, sfi_p, sfuh_p, sflh_p, slslope_p, qtslope_p) &
       bind(C, name="eddy_diff_sfdiag_native_test_c")
    ...
    integer :: mode_local, stratus_mode_local, ntop_turb_local, nbot_turb_local, ncol_local, pcols_local, pver_local
    real(r8), pointer :: qt_local(:,:), ql_local(:,:), sl_local(:,:), pi_local(:,:), pm_local(:,:), zi_local(:,:), cld_local(:,:)
    real(r8), pointer :: sfi_local(:,:), sfuh_local(:,:), sflh_local(:,:), slslope_local(:,:), qtslope_local(:,:)

    mode_local = int(mode_c)
    stratus_mode_local = int(stratus_mode_c)
    ntop_turb_local = int(ntop_turb_c)
    nbot_turb_local = int(nbot_turb_c)
    ncol_local = int(ncol_c)
    pcols_local = int(pcols_c)
    pver_local = int(pver_c)

    call c_f_pointer(qt_p, qt_local, [pcols_local, pver_local])
    call c_f_pointer(ql_p, ql_local, [pcols_local, pver_local])
    call c_f_pointer(sl_p, sl_local, [pcols_local, pver_local])
    call c_f_pointer(pi_p, pi_local, [pcols_local, pver_local + 1])
    call c_f_pointer(pm_p, pm_local, [pcols_local, pver_local])
    call c_f_pointer(zi_p, zi_local, [pcols_local, pver_local + 1])
    call c_f_pointer(cld_p, cld_local, [pcols_local, pver_local])
    call c_f_pointer(sfi_p, sfi_local, [pcols_local, pver_local + 1])
    call c_f_pointer(sfuh_p, sfuh_local, [pcols_local, pver_local])
    call c_f_pointer(sflh_p, sflh_local, [pcols_local, pver_local])
    call c_f_pointer(slslope_p, slslope_local, [pcols_local, pver_local])
    call c_f_pointer(qtslope_p, qtslope_local, [pcols_local, pver_local])

    call sfdiag_driver(mode_local, stratus_mode_local, ntop_turb_local, nbot_turb_local, pcols_local, pver_local, ncol_local, &
         real(qmin_c, r8), real(cpair_c, r8), real(g_c, r8), qt_local, ql_local, sl_local, pi_local, pm_local, zi_local, &
  ...
  subroutine eddy_diff_trbintd_core_select_impl()
    ...
    if (masterproc) then
       if (use_native_trbintd_core_impl) then
          ...
          write(*,*) 'eddy_diff_trbintd_core implementation = native'
          call eddy_diff_append_impl_trace('eddy_diff_trbintd_core implementation = native')
       else
          write(iulog,*) 'eddy_diff_trbintd_core implementation = codon'
          write(*,*) 'eddy_diff_trbintd_core implementation = codon'
          call eddy_diff_append_impl_trace('eddy_diff_trbintd_core implementation = codon')
       end if
    end if
  ...
  subroutine eddy_diff_trbintd_core(ncol, pcols, pver, t_local, z_local, u_local, v_local, qv_local, ql_local, qi_local, &
    ...
    real(r8), target, intent(in) :: pi_local(pcols,pver+1), zi_local(pcols,pver+1)
    ...
    interface
       subroutine eddy_diff_trbintd_core_batch_codon(ncol_c, pcols_c, pver_c, mode_c, stratus_mode_c, ntop_turb_c, &
            nbot_turb_c, qmin_c, cpair_c, latvap_c, latsub_c, g_c, zvir_c, ntzero_c, t_p, z_p, u_p, v_p, qv_p, ql_p, &
            qi_p, gam_p, pmid_p, pi_p, zi_p, cld_p, qt_p, sl_p, slv_p, slslope_p, qtslope_p, dsldp_b_p, dqtdp_b_p, &
            chu_p, chs_p, cmu_p, cms_p, sfi_p, sfuh_p, sflh_p, n2_p, s2_p, ri_p) bind(c, name="eddy_diff_trbintd_core_batch_codon")
         use iso_c_binding, only: c_double, c_int64_t, c_ptr
         integer(c_int64_t), value :: ncol_c, pcols_c, pver_c, mode_c, stratus_mode_c, ntop_turb_c, nbot_turb_c
         real(c_double), value :: qmin_c, cpair_c, latvap_c, latsub_c, g_c, zvir_c, ntzero_c
         type(c_ptr), value :: t_p, z_p, u_p, v_p, qv_p, ql_p, qi_p, gam_p, pmid_p, pi_p, zi_p, cld_p
         type(c_ptr), value :: qt_p, sl_p, slv_p, slslope_p, qtslope_p, dsldp_b_p, dqtdp_b_p
         type(c_ptr), value :: chu_p, chs_p, cmu_p, cms_p, sfi_p, sfuh_p, sflh_p, n2_p, s2_p, ri_p
       end subroutine eddy_diff_trbintd_core_batch_codon
    end interface
    ...
    call eddy_diff_trbintd_core_batch_select_impl()

    if (.not. use_native_trbintd_core_batch_impl) then
       call eddy_diff_trbintd_core_batch_log_entered()
       call eddy_diff_trbintd_core_batch_codon( &
            int(ncol, c_int64_t), int(pcols, c_int64_t), int(pver, c_int64_t), &
            int(sfdiag_resolve_mode(), c_int64_t), int(sfdiag_resolve_stratus_mode(), c_int64_t), &
            int(ntop_turb, c_int64_t), int(nbot_turb, c_int64_t), real(qmin, c_double), real(cpair, c_double), &
            real(latvap, c_double), real(latsub, c_double), real(g, c_double), real(zvir, c_double), real(ntzero, c_double), &
            c_loc(t_local), c_loc(z_local), c_loc(u_local), c_loc(v_local), c_loc(qv_local), c_loc(ql_local), &
            c_loc(qi_local), c_loc(gam_local), c_loc(pmid_local), c_loc(pi_local), c_loc(zi_local), c_loc(cld_local), &
            c_loc(qt_local), c_loc(sl_local), c_loc(slv_local), c_loc(slslope_local), c_loc(qtslope_local), &
            c_loc(dsldp_b_local), c_loc(dqtdp_b_local), c_loc(chu_local), c_loc(chs_local), c_loc(cmu_local), &
            c_loc(cms_local), c_loc(sfi_local), c_loc(sfuh_local), c_loc(sflh_local), c_loc(n2_local), c_loc(s2_local), &
            c_loc(ri_local))
       return
    end if

    call eddy_diff_trbintd_midpoint(ncol, pcols, pver, t_local, z_local, qv_local, ql_local, qi_local, gam_local, qt_local, &
         sl_local, slv_local, chu_local, chs_local, cmu_local, cms_local)

    call eddy_diff_trbintd_slopes(ncol, pcols, pver, pmid_local, sl_local, qt_local, slslope_local, qtslope_local, &
         dsldp_b_local, dqtdp_b_local)

    call eddy_diff_trbintd_sfdiag_interface(ncol, pcols, pver, ql_local, sl_local, qt_local, pi_local, pmid_local, zi_local, &
         cld_local, slslope_local, qtslope_local, u_local, v_local, z_local, chu_local, chs_local, cmu_local, cms_local, &
         sfi_local, sfuh_local, sflh_local, n2_local, s2_local, ri_local)
  ...
  subroutine eddy_diff_trbintd_core_native(ncol, pcols, pver, t_local, z_local, u_local, v_local, qv_local, ql_local, qi_local, &
    ...
    integer :: sfdiag_mode_local, sfdiag_stratus_mode_local
    ...
    sfdiag_mode_local = sfdiag_resolve_mode()
    sfdiag_stratus_mode_local = sfdiag_resolve_stratus_mode()

    call sfdiag_native(sfdiag_mode_local, sfdiag_stratus_mode_local, pcols, pver, ncol, qt_local, ql_local, sl_local, pi_local, &
         pmid_local, zi_local, cld_local, sfi_local, sfuh_local, sflh_local, slslope_local, qtslope_local)

    call eddy_diff_trbintd_sfdiag_interface_native(ncol, pcols, pver, sl_local, qt_local, u_local, v_local, z_local, chu_local, &
         chs_local, cmu_local, cms_local, sfi_local, n2_local, s2_local, ri_local)
  ...
  subroutine eddy_diff_trbintd_sfdiag_interface_select_impl()
    ...
    if (masterproc) then
       if (use_native_trbintd_sfdiag_interface_impl) then
          ...
          write(*,*) 'eddy_diff_trbintd_sfdiag_interface implementation = native'
          call eddy_diff_append_impl_trace('eddy_diff_trbintd_sfdiag_interface implementation = native')
       else
          write(iulog,*) 'eddy_diff_trbintd_sfdiag_interface implementation = codon'
          write(*,*) 'eddy_diff_trbintd_sfdiag_interface implementation = codon'
          call eddy_diff_append_impl_trace('eddy_diff_trbintd_sfdiag_interface implementation = codon')
       end if
    end if
  ...
  subroutine eddy_diff_trbintd_sfdiag_interface(ncol, pcols, pver, ql_local, sl_local, qt_local, pi_local, pmid_local, &
    ...
    interface
       subroutine eddy_diff_trbintd_sfdiag_interface_codon(ncol_c, pcols_c, pver_c, ntzero_c, u_p, v_p, z_p, sl_p, qt_p, &
            chu_p, chs_p, cmu_p, cms_p, sfi_p, n2_p, s2_p, ri_p) &
            bind(c, name="eddy_diff_trbintd_sfdiag_interface_codon")
         use iso_c_binding, only: c_double, c_int64_t, c_ptr
         integer(c_int64_t), value :: ncol_c, pcols_c, pver_c
         real(c_double), value :: ntzero_c
         type(c_ptr), value :: u_p, v_p, z_p, sl_p, qt_p, chu_p, chs_p, cmu_p, cms_p, sfi_p, n2_p, s2_p, ri_p
    ...
    if (use_native_trbintd_sfdiag_interface_impl) then
       call eddy_diff_trbintd_sfdiag_interface_native(ncol, pcols, pver, sl_local, qt_local, u_local, v_local, z_local, &
            chu_local, chs_local, cmu_local, cms_local, sfi_local, n2_local, s2_local, ri_local)
       return
    end if

    call eddy_diff_trbintd_sfdiag_interface_codon(int(ncol, c_int64_t), int(pcols, c_int64_t), int(pver, c_int64_t), &
         real(ntzero, c_double), c_loc(u_local), c_loc(v_local), c_loc(z_local), c_loc(sl_local), c_loc(qt_local), &
         c_loc(chu_local), c_loc(chs_local), c_loc(cmu_local), c_loc(cms_local), c_loc(sfi_local), c_loc(n2_local), &
         c_loc(s2_local), c_loc(ri_local))
  ...
  subroutine eddy_diff_trbintd_sfdiag_interface_native(ncol, pcols, pver, sl_local, qt_local, u_local, v_local, z_local, &
       chu_local, chs_local, cmu_local, cms_local, sfi_local, n2_local, s2_local, ri_local)

    implicit none

    integer, intent(in) :: ncol, pcols, pver
    real(r8), intent(in) :: sl_local(pcols,pver), qt_local(pcols,pver), u_local(pcols,pver), v_local(pcols,pver), z_local(pcols,pver)
    real(r8), intent(inout) :: chu_local(pcols,pver+1), chs_local(pcols,pver+1), cmu_local(pcols,pver+1), cms_local(pcols,pver+1)
    real(r8), intent(in) :: sfi_local(pcols,pver+1)
    real(r8), intent(out) :: n2_local(pcols,pver), s2_local(pcols,pver), ri_local(pcols,pver)
...
222 continue
  ...
  subroutine caleddy_select_impl()
    ...
    if (status == 0 .and. n > 0) then
       ...
       use_native_caleddy_impl = trim(adjustl(impl_name(:n))) == 'native'
    else
       use_native_caleddy_impl = .false.
    end if
  ...
  subroutine caleddy_full_log_entered()
    ...
    integer(i4), target :: ncvfin(pcols)              ! Total number of CL in column
    ...
    integer(i4), target :: kbase(pcols,ncvmax)        ! Vertical index of CL base interface
    integer(i4), target :: ktop(pcols,ncvmax)         ! Vertical index of CL top interface
    ...
    integer :: use_dw_surf_mode                       ! Encoded use_dw_surf for Codon shell
    integer :: choice_tkes_ebprod_mode               ! Encoded choice_tkes=ebprod for Codon shell
    integer :: radf_mode                              ! Encoded choice_radf for Codon shell
    integer :: srcl_mode                              ! Encoded choice_SRCL for Codon shell
    ...
    interface
       subroutine eddy_diff_caleddy_codon(pcols_c, pver_c, ncol_c, ncvmax_c, ntop_turb_c, nbot_turb_c, qrlzero_mode_c, &
            cldeff_mode_c, tkes_mode_c, use_kvf_mode_c, tunl_mode_c, leng_mode_c, evhc_mode_c, wstarent_mode_c, &
            sedfact_mode_c, srcl_mode_c, radf_mode_c, use_dw_surf_mode_c, choice_tkes_ebprod_mode_c, qmin_c, ricrit_c, &
            b1_c, vk_c, ntzero_c, alph1_c, alph2_c, alph3_c, alph4_c, alph5_c, alph4exs_c, ghmin_c, tunl_c, ctunl_c, &
            cleng_c, lbulk_max_c, tkemax_c, rinc_c, ae_c, a1l_c, a1i_c, ccrit_c, wstar3factcrit_c, onet_c, rcapmin_c, &
            rcapmax_c, wfac_c, wpertmin_c, tfac_c, gravit_c, cpair_c, latvap_c, a2l_c, a3l_c, jbumin_c, evhcmax_c, &
            ased_c, fak_c, sl_p, qt_p, ql_p, slv_p, u_p, v_p, pi_p, z_p, zi_p, qflx_p, shflx_p, slslope_p, qtslope_p, &
            chu_p, chs_p, cmu_p, cms_p, sfuh_p, sflh_p, n2_p, s2_p, ri_p, rrho_p, pblh_p, ustar_p, kvh_in_p, kvm_in_p, &
            kvh_p, kvm_p, tpert_p, qpert_p, qrlin_p, kvf_p, tke_p, bprod_p, sprod_p, wpert_p, tkes_p, went_p, &
            turbtype_p, sm_aw_p, kbase_o_p, ktop_o_p, ncvfin_o_p, kbase_mg_p, ktop_mg_p, ncvfin_mg_p, kbase_f_p, &
            ktop_f_p, ncvfin_f_p, wet_CL_p, web_CL_p, jtbu_CL_p, jbbu_CL_p, evhc_CL_p, jt2slv_CL_p, n2ht_CL_p, &
            n2hb_CL_p, lwp_CL_p, opt_depth_CL_p, radinvfrac_CL_p, radf_CL_p, wstar_CL_p, wstar3fact_CL_p, ebrk_p, &
            wbrk_p, lbrk_p, ricl_p, ghcl_p, shcl_p, smcl_p, gh_a_p, sh_a_p, sm_a_p, ri_a_p, leng_p, wcap_p, pblhp_p, &
            cld_p, ipbl_p, kpblh_p, wsedl_p, wsed_CL_p, leng_max_p, qrlw_p, cldeff_p, bflxs_p, ncvfin_p, kbase_p, &
            ktop_p, belong_mask_p, stlmask_p, extend_p, extend_up_p, extend_dn_p, zisocl_status_p, ncvsurf_p, &
            srcl_status_p, zero_tke_mask_p, closure_status_p, caleddy_status_p) bind(c, name="eddy_diff_caleddy_codon")
         use iso_c_binding, only: c_double, c_int64_t, c_ptr
         integer(c_int64_t), value :: pcols_c, pver_c, ncol_c, ncvmax_c, ntop_turb_c, nbot_turb_c, qrlzero_mode_c, &
              cldeff_mode_c, tkes_mode_c, use_kvf_mode_c, tunl_mode_c, leng_mode_c, evhc_mode_c, wstarent_mode_c, &
              sedfact_mode_c, srcl_mode_c, radf_mode_c, use_dw_surf_mode_c, choice_tkes_ebprod_mode_c
         real(c_double), value :: qmin_c, ricrit_c, b1_c, vk_c, ntzero_c, alph1_c, alph2_c, alph3_c, alph4_c, &
              alph5_c, alph4exs_c, ghmin_c, tunl_c, ctunl_c, cleng_c, lbulk_max_c, tkemax_c, rinc_c, ae_c, a1l_c, &
              a1i_c, ccrit_c, wstar3factcrit_c, onet_c, rcapmin_c, rcapmax_c, wfac_c, wpertmin_c, tfac_c, gravit_c, &
              cpair_c, latvap_c, a2l_c, a3l_c, jbumin_c, evhcmax_c, ased_c, fak_c
         type(c_ptr), value :: sl_p, qt_p, ql_p, slv_p, u_p, v_p, pi_p, z_p, zi_p, qflx_p, shflx_p, slslope_p, &
              qtslope_p, chu_p, chs_p, cmu_p, cms_p, sfuh_p, sflh_p, n2_p, s2_p, ri_p, rrho_p, pblh_p, ustar_p, &
              kvh_in_p, kvm_in_p, kvh_p, kvm_p, tpert_p, qpert_p, qrlin_p, kvf_p, tke_p, bprod_p, sprod_p, wpert_p, &
              tkes_p, went_p, turbtype_p, sm_aw_p, kbase_o_p, ktop_o_p, ncvfin_o_p, kbase_mg_p, ktop_mg_p, &
              ncvfin_mg_p, kbase_f_p, ktop_f_p, ncvfin_f_p, wet_CL_p, web_CL_p, jtbu_CL_p, jbbu_CL_p, evhc_CL_p, &
              jt2slv_CL_p, n2ht_CL_p, n2hb_CL_p, lwp_CL_p, opt_depth_CL_p, radinvfrac_CL_p, radf_CL_p, wstar_CL_p, &
              wstar3fact_CL_p, ebrk_p, wbrk_p, lbrk_p, ricl_p, ghcl_p, shcl_p, smcl_p, gh_a_p, sh_a_p, sm_a_p, &
              ri_a_p, leng_p, wcap_p, pblhp_p, cld_p, ipbl_p, kpblh_p, wsedl_p, wsed_CL_p, leng_max_p, qrlw_p, &
              cldeff_p, bflxs_p, ncvfin_p, kbase_p, ktop_p, belong_mask_p, stlmask_p, extend_p, extend_up_p, &
              extend_dn_p, zisocl_status_p, ncvsurf_p, srcl_status_p, zero_tke_mask_p, closure_status_p, &
              caleddy_status_p
    ...
    if (use_native_caleddy_impl) then
       ...
       call eddy_diff_caleddy_setup_batch_call(ncol, pcols, pver, ncvmax, ntop_turb, qrlzero_mode, cldeff_mode, tkes_mode, &
            use_kvf_mode, qmin, vk, ql, qrlin, cld, kvf, kvh_in, kvm_in, n2, s2, shflx, qflx, rrho, ustar, z, chu, chs, &
            cmu, cms, sflh, qrlw, cldeff, kvh, kvm, bflxs, bprod, sprod, wcap, leng, tke, turbtype, went, wet_CL, web_CL, &
            jtbu_CL, jbbu_CL, evhc_CL, jt2slv_CL, n2ht_CL, n2hb_CL, lwp_CL, opt_depth_CL, radinvfrac_CL, radf_CL, wstar_CL, &
            wstar3fact_CL, ricl, ghcl, shcl, smcl, ebrk, wbrk, lbrk, gh_a, sh_a, sm_a, ri_a, sm_aw, ipbl, kpblh, wsed_CL, &
            ri, minpblh, zi, ktop, kbase, ncvfin)
    end if
  ...
  subroutine zisocl_select_impl()

    character(len=32) :: impl_name
    integer :: status, n, i_char, code
    ...
    if (status == 0 .and. n > 0) then
       do i_char = 1, n
          code = iachar(impl_name(i_char:i_char))
          if (code >= iachar('A') .and. code <= iachar('Z')) then
             impl_name(i_char:i_char) = achar(code + iachar('a') - iachar('A'))
          end if
       end do
       use_native_zisocl_impl = trim(adjustl(impl_name(:n))) == 'native'
    else
       use_native_zisocl_impl = .false.
    end if

    zisocl_impl_selected = .true.
  ...
  end subroutine zisocl_select_impl
  ...
  !============================================================================== !
    ...
    logical :: saved_use_native_zisocl_surface_state_impl, saved_zisocl_surface_state_impl_selected
    logical :: saved_use_native_zisocl_surface_extend_impl, saved_zisocl_surface_extend_impl_selected
    logical :: saved_use_native_zisocl_sbcl_state_impl, saved_zisocl_sbcl_state_impl_selected
    logical :: saved_use_native_zisocl_initial_state_impl, saved_zisocl_initial_state_impl_selected
    logical :: saved_use_native_zisocl_extended_state_impl, saved_zisocl_extended_state_impl_selected
    logical :: saved_use_native_zisocl_non_sbcl_state_impl, saved_zisocl_non_sbcl_state_impl_selected
    logical :: saved_use_native_zisocl_upward_state_impl, saved_zisocl_upward_state_impl_selected
    logical :: saved_use_native_zisocl_downward_state_impl, saved_zisocl_downward_state_impl_selected
    ...
    saved_use_native_zisocl_surface_state_impl = use_native_zisocl_surface_state_impl
    saved_zisocl_surface_state_impl_selected = zisocl_surface_state_impl_selected
    saved_use_native_zisocl_surface_extend_impl = use_native_zisocl_surface_extend_impl
    saved_zisocl_surface_extend_impl_selected = zisocl_surface_extend_impl_selected
    saved_use_native_zisocl_sbcl_state_impl = use_native_zisocl_sbcl_state_impl
    saved_zisocl_sbcl_state_impl_selected = zisocl_sbcl_state_impl_selected
    saved_use_native_zisocl_initial_state_impl = use_native_zisocl_initial_state_impl
    saved_zisocl_initial_state_impl_selected = zisocl_initial_state_impl_selected
    saved_use_native_zisocl_extended_state_impl = use_native_zisocl_extended_state_impl
    saved_zisocl_extended_state_impl_selected = zisocl_extended_state_impl_selected
    saved_use_native_zisocl_non_sbcl_state_impl = use_native_zisocl_non_sbcl_state_impl
    saved_zisocl_non_sbcl_state_impl_selected = zisocl_non_sbcl_state_impl_selected
    saved_use_native_zisocl_upward_state_impl = use_native_zisocl_upward_state_impl
    saved_zisocl_upward_state_impl_selected = zisocl_upward_state_impl_selected
    saved_use_native_zisocl_downward_state_impl = use_native_zisocl_downward_state_impl
    saved_zisocl_downward_state_impl_selected = zisocl_downward_state_impl_selected
    saved_use_native_zisocl_stability_impl = use_native_zisocl_stability_impl
    saved_zisocl_stability_impl_selected = zisocl_stability_impl_selected
    saved_use_native_zisocl_layer_energy_impl = use_native_zisocl_layer_energy_impl
    saved_zisocl_layer_energy_impl_selected = zisocl_layer_energy_impl_selected
    saved_use_native_zisocl_interface_energy_impl = use_native_zisocl_interface_energy_impl
    saved_zisocl_interface_energy_impl_selected = zisocl_interface_energy_impl_selected

    use_native_zisocl_surface_energy_impl = use_native_zisocl_impl
    zisocl_surface_energy_impl_selected = .true.
    use_native_zisocl_surface_state_impl = use_native_zisocl_impl
    zisocl_surface_state_impl_selected = .true.
    use_native_zisocl_surface_extend_impl = use_native_zisocl_impl
    zisocl_surface_extend_impl_selected = .true.
    use_native_zisocl_sbcl_state_impl = use_native_zisocl_impl
    zisocl_sbcl_state_impl_selected = .true.
    use_native_zisocl_initial_state_impl = use_native_zisocl_impl
    zisocl_initial_state_impl_selected = .true.
    use_native_zisocl_extended_state_impl = use_native_zisocl_impl
    zisocl_extended_state_impl_selected = .true.
    use_native_zisocl_non_sbcl_state_impl = use_native_zisocl_impl
    zisocl_non_sbcl_state_impl_selected = .true.
    use_native_zisocl_upward_state_impl = use_native_zisocl_impl
    zisocl_upward_state_impl_selected = .true.
    use_native_zisocl_downward_state_impl = use_native_zisocl_impl
    zisocl_downward_state_impl_selected = .true.
    use_native_zisocl_stability_impl = use_native_zisocl_impl
    zisocl_stability_impl_selected = .true.
    use_native_zisocl_layer_energy_impl = use_native_zisocl_impl
    zisocl_layer_energy_impl_selected = .true.
    ...
    use_native_zisocl_surface_energy_impl = saved_use_native_zisocl_surface_energy_impl
    zisocl_surface_energy_impl_selected = saved_zisocl_surface_energy_impl_selected
    use_native_zisocl_surface_state_impl = saved_use_native_zisocl_surface_state_impl
    zisocl_surface_state_impl_selected = saved_zisocl_surface_state_impl_selected
    use_native_zisocl_surface_extend_impl = saved_use_native_zisocl_surface_extend_impl
    zisocl_surface_extend_impl_selected = saved_zisocl_surface_extend_impl_selected
    use_native_zisocl_sbcl_state_impl = saved_use_native_zisocl_sbcl_state_impl
    zisocl_sbcl_state_impl_selected = saved_zisocl_sbcl_state_impl_selected
    use_native_zisocl_initial_state_impl = saved_use_native_zisocl_initial_state_impl
    zisocl_initial_state_impl_selected = saved_zisocl_initial_state_impl_selected
    use_native_zisocl_extended_state_impl = saved_use_native_zisocl_extended_state_impl
    zisocl_extended_state_impl_selected = saved_zisocl_extended_state_impl_selected
    use_native_zisocl_non_sbcl_state_impl = saved_use_native_zisocl_non_sbcl_state_impl
    zisocl_non_sbcl_state_impl_selected = saved_zisocl_non_sbcl_state_impl_selected
    use_native_zisocl_upward_state_impl = saved_use_native_zisocl_upward_state_impl
    zisocl_upward_state_impl_selected = saved_zisocl_upward_state_impl_selected
    use_native_zisocl_downward_state_impl = saved_use_native_zisocl_downward_state_impl
    zisocl_downward_state_impl_selected = saved_zisocl_downward_state_impl_selected
    use_native_zisocl_stability_impl = saved_use_native_zisocl_stability_impl
    zisocl_stability_impl_selected = saved_zisocl_stability_impl_selected
    use_native_zisocl_layer_energy_impl = saved_use_native_zisocl_layer_energy_impl
    zisocl_layer_energy_impl_selected = saved_zisocl_layer_energy_impl_selected
    use_native_zisocl_interface_energy_impl = saved_use_native_zisocl_interface_energy_impl
    zisocl_interface_energy_impl_selected = saved_zisocl_interface_energy_impl_selected
    ...
    subroutine zisocl_native( pcols  , pver  , long ,                                 & 
                       z      , zi    , n2   ,  s2      ,                      & 
                       bprod  , sprod , bflxs,  tkes    ,                      & 
                       ncvfin , kbase , ktop ,  belongcv,                      & 
                       ricl   , ghcl  , shcl ,  smcl    ,                      &
    ...
    integer(i4), intent(inout) :: kbase(pcols,ncvmax) ! Base external interface index of CL
    integer(i4), intent(inout) :: ktop(pcols,ncvmax)  ! Top external interface index of CL
    integer(i4), intent(inout) :: ncvfin(pcols)       ! Total number of CLs
