...
use ap_nucleate_ice_cam_calc_scheme, only: nucleate_ice_cam_calc_run
...
subroutine nucleate_ice_cam_calc( &
   ...
   real(r8) :: so4_num_field(pcols,pver)
   real(r8) :: dst_num_field(pcols,pver)
   real(r8) :: soot_num_field(pcols,pver)
   real(r8) :: wght
   real(r8) :: dmc
   real(r8) :: ssmc
   ...
   call t_startf('ap_nucleate_ice_cam_calc_run')
   ...
   do k = top_lev, pver
      do i = 1, ncol
         ...
         if (t(i,k) < tmelt - 5._r8) then
            ...
            so4_num_field(i,k) = so4_num
            dst_num_field(i,k) = dst_num
            soot_num_field(i,k) = soot_num

         end if
      end do
   end do

   call nucleate_ice_cam_calc_run( &
        pcols, pver, ncol, top_lev, tmelt, use_preexisting_ice, &
        wsubi, t, pmid, relhum, icldm, qc, qi, ni, rho, &
        so4_num_field, dst_num_field, soot_num_field, &
   ...
   call t_stopf('ap_nucleate_ice_cam_calc_run')
