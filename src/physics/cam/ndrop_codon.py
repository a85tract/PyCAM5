...
def dropmixnuc_codon(
    ...
    ccn_p: cobj,
    ccn_super_p: cobj,
    ccn_species_raer_ptrs_p: cobj,
    ccn_species_qqcw_ptrs_p: cobj,
    ccn_species_specdens_p: cobj,
    ccn_species_spechygro_p: cobj,
    ccn_num_raer_ptrs_p: cobj,
    ccn_num_qqcw_ptrs_p: cobj,
    ccn_naerosol_p: cobj,
    ccn_vaerosol_p: cobj,
    ccn_hygro_p: cobj,
    ccn_amcube_p: cobj,
    ...
    qs_act_all = Ptr[float](qs_act_all_p)
    qs_act = Ptr[float](qs_act_p)
    taumix_internal_pver_inv_ref = Ptr[float](taumix_internal_pver_inv_p)
    nsubmix_ref = Ptr[i32](nsubmix_p)
    dtmix_ref = Ptr[float](dtmix_p)
    nsav_ref = Ptr[i32](nsav_p)
    nnew_ref = Ptr[i32](nnew_p)

    ndrop_dropmixnuc_zero_fields_codon(pcols, pver, ntot_amode, factnum_p, wtke_p)

    for i in range(1, ncol + 1):
        ndrop_dropmixnuc_column_init_codon(
            i,
            pcols,
            pver,
            top_lev,
        ...
        nsav_ref[0] = i32(1)
        nnew_ref[0] = i32(2)
        ndrop_dropmixnuc_aero_column_copy_all_codon(
            i,
            pcols,
            pver,
            top_lev,
        ...
        for k in range(top_lev, pver + 1):
            qs_act[k - 1] = qs_act_all[_idx2(i, k, pcols)]

        taumix_internal_pver_inv_ref[0] = 0.0
        status = ndrop_dropmixnuc_activation_loops_codon(
            i,
            pcols,
            pver,
            top_lev,
        ...
        ndrop_dropmixnuc_mix_setup_codon(
            ...
            ntot_amode,
            dtmicro,
            taumix_internal_pver_inv_ref[0],
            cldn_p,
            zs_p,
            zn_p,
            csbot_p,
        ...
        nsubmix = int(nsubmix_ref[0])
        for n in range(1, nsubmix + 1):
            ndrop_dropmixnuc_submix_all_codon(
                pver,
                top_lev,
                ntot_amode,
                ncnst_tot,
        ...
        ndrop_dropmixnuc_evaporate_clear_layers_codon(
            i,
            pcols,
            pver,
            top_lev,
        ...
        ndrop_dropmixnuc_finalize_column_codon(
            i,
            pcols,
            pver,
            top_lev,
        ...
        if prog_modal_aero != 0:
            ndrop_dropmixnuc_aero_tend_all_codon(
                i,
                pcols,
                psetcols,
                pver,
    ...
    ndrop_ccncalc_direct_codon(
        ...
        smcoefcoef,
        nspec_amode_p,
        ccn_species_raer_ptrs_p,
        ccn_species_qqcw_ptrs_p,
        ccn_species_specdens_p,
        ccn_species_spechygro_p,
        ccn_num_raer_ptrs_p,
        ccn_num_qqcw_ptrs_p,
        voltonumblo_p,
        voltonumbhi_p,
