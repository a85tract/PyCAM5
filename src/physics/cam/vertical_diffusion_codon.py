...
def vertical_diffusion_tend_misc_batch_codon(
    ...
    vertical_diffusion_tend_misc_stage_dispatch_codon(
        stage,
        ncol,
        pcols,
        pver,
...
@export
def vertical_diffusion_core_stage_dispatch_codon(
    stage: int,
    ncol: int,
    pcols: int,
    pver: int,
    ...
    if stage == 1:
        vertical_diffusion_modal_aero_flux_codon(
            ncol,
            pcols,
            pver,
            pcnst,
        ...
        vertical_diffusion_flux_diag_codon(
            ncol,
            pcols,
            pver,
            pverp,
        ...
        vertical_diffusion_ptend_core_codon(
            ncol,
            pcols,
            pver,
            pcnst,
...
@export
def vertical_diffusion_core_batch_codon(
    stage: int,
    ncol: int,
    pcols: int,
    pver: int,
    ...
    vertical_diffusion_core_stage_dispatch_codon(
        stage,
        ncol,
        pcols,
        pver,
...
def _eddy_diff_caleddy_light_batch_dispatch(
    stage: int,
    i_col: int,
    pcols: int,
    pver: int,
...
def eddy_diff_caleddy_light_batch_codon(
    ...
    _eddy_diff_caleddy_light_batch_dispatch(
        stage,
        i_col,
        pcols,
        pver,
...
def _eddy_diff_caleddy_core_batch_zisocl_dispatch(
    i_col: int,
    pcols: int,
    pver: int,
    ncvmax: int,
...
def eddy_diff_caleddy_core_batch_zisocl_codon(
    ...
    _eddy_diff_caleddy_core_batch_zisocl_dispatch(
        i_col,
        pcols,
        pver,
        ncvmax,
...
def eddy_diff_caleddy_codon(
    ...
    srcl_mode: int,
    radf_mode: int,
    use_dw_surf_mode: int,
    choice_tkes_ebprod_mode: int,
    qmin: float,
    ricrit: float,
    b1: float,
    vk: float,
    ...
    extend_up_p: cobj,
    extend_dn_p: cobj,
    zisocl_status_p: cobj,
    ncvsurf_p: cobj,
    srcl_status_p: cobj,
    zero_tke_mask_p: cobj,
    closure_status_p: cobj,
    caleddy_status_p: cobj,
):
    ncvfin = Ptr[i32](ncvfin_p)
    kbase = Ptr[i32](kbase_p)
    belong_mask = Ptr[i32](belong_mask_p)
    ...
    caleddy_status[1] = i32(0)
    caleddy_status[2] = i32(0)

    eddy_diff_caleddy_init_codon(
        ncol,
        pcols,
        pver,
        qrlzero_mode,
    ...
    eddy_diff_caleddy_diaginit_codon(
        ncol,
        pcols,
        pver,
        ncvmax,
    ...
    for i in range(1, ncol + 1):
        eddy_diff_caleddy_regime_diag_codon(
            i, pcols, ncvmax, kbase_p, ktop_p, ncvfin_p, kbase_o_p, ktop_o_p, ncvfin_o_p
        )

        eddy_diff_caleddy_surface_tke_codon(
            i, pcols, pver, b1, vk, tkemax, z_p, bprod_p, sprod_p, tkes_p, tke_p, wcap_p
        )

        ncvsurf_state[0] = i32(0)
        if int(ncvfin[i - 1]) > 0:
            eddy_diff_zisocl_codon(
                i,
                pcols,
            ...
            if zisocl_status[0] != i32(0):
                caleddy_status[0] = i32(1)
                caleddy_status[1] = zisocl_status[0]
                caleddy_status[2] = i32(i)
                return
            if int(kbase[_idx2(i, 1, pcols)]) == pver + 1:
                ncvsurf_state[0] = i32(1)

        eddy_diff_caleddy_regime_diag_codon(
            i, pcols, ncvmax, kbase_p, ktop_p, ncvfin_p, kbase_mg_p, ktop_mg_p, ncvfin_mg_p
        )

        eddy_diff_caleddy_srcl_codon(
            i,
            pcols,
            pver,
            ncvmax,
        ...
        if srcl_status[0] != i32(0):
            caleddy_status[0] = i32(2)
            caleddy_status[1] = srcl_status[0]
            caleddy_status[2] = i32(i)
            return

        eddy_diff_caleddy_regime_diag_codon(
            i, pcols, ncvmax, kbase_p, ktop_p, ncvfin_p, kbase_f_p, ktop_f_p, ncvfin_f_p
        )

        eddy_diff_compute_radf_codon(
            i,
            pcols,
            pver,
            ncvmax,
        ...
        eddy_diff_caleddy_closure_codon(
            i,
            pcols,
            pver,
            ncvmax,
...
def _eddy_diff_caleddy_setup_batch_dispatch(
    ncol: int,
    pcols: int,
    pver: int,
    ncvmax: int,
...
def eddy_diff_caleddy_setup_batch_codon(
    ...
    _eddy_diff_caleddy_setup_batch_dispatch(
        ncol,
        pcols,
        pver,
        ncvmax,
...
def _eddy_diff_caleddy_cloud_rad_batch_dispatch(
    stage: int,
    i_col: int,
    pcols: int,
    pver: int,
...
def eddy_diff_caleddy_cloud_rad_batch_codon(
    ...
    _eddy_diff_caleddy_cloud_rad_batch_dispatch(
        stage,
        i_col,
        pcols,
        pver,
...
def _eddy_diff_caleddy_post_batch_dispatch(
    stage: int,
    i_col: int,
    pcols: int,
    pver: int,
...
def eddy_diff_caleddy_post_batch_codon(
    ...
    _eddy_diff_caleddy_post_batch_dispatch(
        stage,
        i_col,
        pcols,
        pver,
...
@export
def eddy_diff_driver_front_batch_codon(
    stage: int,
    ncol: int,
    pcols: int,
    pver: int,
...
def eddy_diff_trbintd_core_stage_dispatch_codon(
    ...
    eddy_diff_trbintd_midpoint_codon(
        ncol,
        pcols,
        pver,
        cpair,
    ...
    eddy_diff_trbintd_slopes_codon(
        ncol,
        pcols,
        pver,
        pmid_p,
    ...
    eddy_diff_trbintd_sfdiag_interface_codon(
        ncol,
        pcols,
        pver,
        ntzero,
...
@export
def vertical_diffusion_diag_stage_dispatch_codon(
    stage: int,
    ncol: int,
    pcols: int,
    pver: int,
    ...
    if stage == 1:
        vertical_diffusion_pre_pbl_diag_codon(
            ncol,
            pcols,
            pver,
            pcnst,
        ...
        vertical_diffusion_pre_qsat_rh_codon(
            ncol,
            pcols,
            pver,
            state_q_p,
        ...
        vertical_diffusion_post_pbl_state_codon(
            ncol,
            pcols,
            pver,
            pcnst,
        ...
        vertical_diffusion_post_qsat_diag_codon(
            ncol,
            pcols,
            pver,
            rztodt,
        ...
        vertical_diffusion_obklen_diag_codon(
            ncol,
            pcols,
            pver,
            cpair,
...
@export
def vertical_diffusion_diag_batch_codon(
    stage: int,
    ncol: int,
    pcols: int,
    pver: int,
    ...
    vertical_diffusion_diag_stage_dispatch_codon(
        stage,
        ncol,
        pcols,
        pver,
