...
def _aero_model_emissions_dust_shell(
    ...
    for i in range(1, ncol + 1):
        for m in range(1, dust_nbin + 1):
            ...
            dust_x_mton = 6.0 / (pi_val * dust_density * (dust_dmt_vwr[m - 1] ** 3.0))
            cflx[_flux_idx(i, inum, pcols)] = value * dust_x_mton
...
def aero_model_emissions_shell_wind_codon(
    ...
    if compute_wind != 0 and (stage == 2 or stage == 3):
        aero_model_emissions_seasalt_wind_codon(
            ncol,
            pcols,
            pver,
            z0,
    ...
    aero_model_emissions_shell_codon(
        stage,
        ncol,
        pcols,
        ndstflx,
