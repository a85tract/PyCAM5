...
subroutine unicon_cam_select_impl()
   ...
   impl_name = 'codon'
   call cam_codon_get_impl('UNICON_CAM_IMPL', impl_name, n, status)
...
subroutine unicon_cam_init(pbuf2d)
   ...
   call unicon_init(latvap, cpair, latice, zvir, rair, gravit, mwh2o/mwdry)
   ...
   call addfld('a_p_SP',    '1', pverp, 'A', 'Convective precipitation area',                                      phys_decomp)
   call addfld('am_evp_SP', '1', pver,  'A', 'Convective evaporation area',                                        phys_decomp)
   call addfld('am_pu_SP',  '1', pver,  'A', 'Overlapping area between conv precipitation and sat updraft area',   phys_decomp)
   call addfld('x_um_SP',   'm', pver,  'A', 'Zonal displacement of the updraft area from the surface',            phys_decomp)
   call addfld('y_um_SP',   'm', pver,  'A', 'Meridional displacement of the updraft area from the surface',       phys_decomp)
   call addfld('x_p_SP',    'm', pverp, 'A', 'Zonal displacement of the precipitation area from the surface',      phys_decomp)
   call addfld('y_p_SP',    'm', pverp, 'A', 'Meridional displacement of the precipitation area from the surface', phys_decomp)
