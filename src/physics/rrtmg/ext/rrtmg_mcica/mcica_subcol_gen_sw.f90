...
!      real(kind=r8) :: ql(ncol,nlay)                 ! liq water (specific humidity)
      ...
      interface
         subroutine mcica_subcol_sw_codon(ncol_c, nlay_c, ld_play_c, ld_size_c, ld_pmid_c, ld_out_c, &
              play_p, rei_p, rel_p, pmid_p, reicmcl_p, relqmcl_p) bind(c, name="mcica_subcol_sw_codon")
         ...
         end subroutine mcica_subcol_sw_codon
      end interface
...
! Indices
      ...
      interface
         subroutine rrtmg_sw_subcol_cldf_prep_codon(ncol_c, nlay_c, ld_cld_c, ld_cldf_c, &
              cldmin_c, cld_p, cldf_p) bind(c, name="rrtmg_sw_subcol_cldf_prep_codon")
         ...
         subroutine rrtmg_sw_subcol_kiss_random_case2_codon(ncol_c, nlay_c, nsubcol_c, &
              seed1_p, seed2_p, seed3_p, seed4_p, cdf_p) &
              bind(c, name="rrtmg_sw_subcol_kiss_random_case2_codon")
            use iso_c_binding, only: c_int64_t, c_ptr
            integer(c_int64_t), value :: ncol_c, nlay_c, nsubcol_c
            type(c_ptr), value :: seed1_p, seed2_p, seed3_p, seed4_p, cdf_p
         end subroutine rrtmg_sw_subcol_kiss_random_case2_codon
         subroutine rrtmg_sw_subcol_kiss_advance_codon(ncol_c, nstep_c, &
              seed1_p, seed2_p, seed3_p, seed4_p) bind(c, name="rrtmg_sw_subcol_kiss_advance_codon")
            use iso_c_binding, only: c_int64_t, c_ptr
            integer(c_int64_t), value :: ncol_c, nstep_c
            type(c_ptr), value :: seed1_p, seed2_p, seed3_p, seed4_p
         end subroutine rrtmg_sw_subcol_kiss_advance_codon
         ...
         end subroutine rrtmg_sw_subcol_overlap_codon
...
! ensure that cloud fractions are in bounds 
      ...
      if (use_native_subcol_fill_sw_impl) then
         ...
         call subcol_fill_sw_log_entered()
         call rrtmg_sw_subcol_cldf_prep_codon( &
...
! Must use pmid from bottom four layers. 
         ...
         if (use_native_subcol_fill_sw_impl) then
            ...
            call subcol_fill_sw_log_entered()
            call rrtmg_sw_subcol_kiss_advance_codon( &
...
! i) pick a random value at every level
         ...
         if (irnd.eq.0) then
            ...
            if (use_native_subcol_fill_sw_impl) then
               ...
               call subcol_fill_sw_log_entered()
               call rrtmg_sw_subcol_kiss_random_case2_codon( &
...
!    - if the layer above is clear, we use a new random number 
         ...
         if (use_native_subcol_fill_sw_impl) then
            ...
            call subcol_fill_sw_log_entered()
            call rrtmg_sw_subcol_overlap_codon( &
                 int(ncol, c_int64_t), int(nlay, c_int64_t), int(nsubcol, c_int64_t), &
...
! --------------------------------------------------------------------------
      ...
      if (masterproc) then
         write(iulog,*) 'rrtmg_sw_subcol_fill entered (mcica sw pressure/size/cldf/seed/advance/random prep, ' // &
