...
!-----------------------------------------------------------------------
    ...
    interface
       function cam_write_restart_codon(tag) result(tag_out) bind(c, name='cam_write_restart_codon')
         import :: c_int64_t
         integer(c_int64_t), value :: tag
         integer(c_int64_t) :: tag_out
       end function cam_write_restart_codon
    end interface
    ...
    rt_codon_impl_name = 'codon'
    call cam_codon_get_impl('CAM_WRITE_RESTART_IMPL', rt_codon_impl_name, rt_codon_n, rt_codon_status)
    if (.not. (rt_codon_status == 0 .and. rt_codon_n > 0 .and. &
         trim(adjustl(rt_codon_impl_name(:rt_codon_n))) == 'native')) then
       rt_codon_tag_out = cam_write_restart_codon(int(216, c_int64_t))
       if (rt_codon_tag_out /= int(216, c_int64_t)) then
          write(iulog,*) 'cam_write_restart_codon tag roundtrip failed'
          stop 2
       endif
       if (.not. rt_codon_proof_seen) then
          write(iulog,*) 'cam_write_restart implementation = codon'
          rt_codon_proof_seen = .true.
       endif
    endif
    !-----------------------------------------------------------------------
      ...
      call init_restart_physics(File, pbuf2d, hdimids)
