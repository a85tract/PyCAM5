module restart_dynamics
  ...
  use pio, only : pio_global, pio_unlimited, pio_double, pio_def_dim, &
         ...
         pio_setdebuglevel, pio_inq_dimid, file_desc_t, pio_offset_kind, &
         pio_get_att, pio_inq_dimid, pio_inq_dimlen, pio_inq_varid, &
         pio_read_darray, pio_setframe, io_desc_t
...
CONTAINS
  ...
  subroutine init_restart_dynamics(File, hdimids, dyn_out)
    ...
    interface
       function init_restart_dynamics_codon(tag) result(tag_out) bind(c, name='init_restart_dynamics_codon')
         import :: c_int64_t
         integer(c_int64_t), value :: tag
         integer(c_int64_t) :: tag_out
       end function init_restart_dynamics_codon
    end interface
    ...
    call cam_codon_get_impl('INIT_RESTART_DYNAMICS_IMPL', rt_codon_impl_name, rt_codon_n, rt_codon_status)
    if (.not. (rt_codon_status == 0 .and. rt_codon_n > 0 .and. &
         trim(adjustl(rt_codon_impl_name(:rt_codon_n))) == 'native')) then
       rt_codon_tag_out = init_restart_dynamics_codon(int(13, c_int64_t))
       if (rt_codon_tag_out /= int(13, c_int64_t)) then
          write(iulog,*) 'init_restart_dynamics_codon tag roundtrip failed'
          stop 2
       endif
       if (.not. rt_codon_proof_seen) then
          write(iulog,*) 'init_restart_dynamics implementation = codon'
...
!   To be used by physics
  ...
  subroutine write_restart_dynamics(File, dyn_out)
    use pio, only : pio_offset_kind, io_desc_t, pio_double, pio_write_darray, &
         pio_put_var, pio_initdecomp, pio_def_dim, &
