...
#include <mpif.h>
    ...
    interface
       function initmpi_codon(tag) result(tag_out) bind(c, name='initmpi_codon')
         import :: c_int64_t
         integer(c_int64_t), value :: tag
         integer(c_int64_t) :: tag_out
       end function initmpi_codon
    end interface
    ...
    call cam_codon_get_impl('INITMPI_IMPL', rt_codon_impl_name, rt_codon_n, rt_codon_status)
    if (.not. (rt_codon_status == 0 .and. rt_codon_n > 0 .and. &
         trim(adjustl(rt_codon_impl_name(:rt_codon_n))) == 'native')) then
       rt_codon_tag_out = initmpi_codon(int(23, c_int64_t))
       if (rt_codon_tag_out /= int(23, c_int64_t)) then
          write(iulog,*) 'initmpi_codon tag roundtrip failed'
          stop 2
       endif
       if (.not. rt_codon_proof_seen) then
          write(iulog,*) 'initmpi implementation = codon'
