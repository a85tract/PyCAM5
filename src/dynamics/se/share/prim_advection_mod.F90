...
subroutine remap_q_ppm_grid_select_impl()
  ...
  if (status == 0 .and. n > 0) then
    ...
    remap_q_ppm_grid_use_native_impl = trim(adjustl(impl_name(:n))) == 'native'
  else
    remap_q_ppm_grid_use_native_impl = .false.
  end if
...
!
  ...
  logical :: euler_step_dssvar_restore_use_native_impl = .false.
...
!             Qtens_biharmonic(i,j,k,q,ie) = elem(ie)%state%Qdp(i,j,k,q,n0_qdp)/dp(i,j,k)
    ...
    if ( rhs_multiplier == 2 ) then
      ...
      do ie = nets, nete
        do q = 1, qsize
          if (.not. euler_step_qtens_biharmonic_unapply_use_native_impl) then
            call euler_step_qtens_biharmonic_unapply_apply_codon( &
                 Qtens_biharmonic(:,:,:,q,ie), elem(ie)%spheremp, dp0, rhs_viss, dt, nu_q &
            )
          else
            do k = 1, nlev
              !OMP_COLLAPSE_SIMD
  ...
  do ie = nets , nete
    ...
    do q = 1 , qsize
      do k = 1 , nlev  !  dp_star used as temporary instead of divdp (AAM)
        ...
        if (.not. euler_step_qtens_base_use_native_impl) then
          call euler_step_qtens_base_apply_codon(elem(ie)%state%Qdp(:,:,k,q,n0_qdp), dp_star(:,:,k), dt, Qtens(:,:,k))
        else
          do j=1,np
            do i=1,np
...
!            dpdiss(:,:) = ( hvcoord%hybi(k+1) - hvcoord%hybi(k) ) * elem(ie)%derived%psdiss_biharmonic(:,:)
      ...
      if (.not. euler_step_qdp_writeback_use_native_impl) then
        call euler_step_qdp_writeback_apply_codon(elem(ie)%state%Qdp, elem(ie)%spheremp, Qtens, q, np1_qdp)
      else
        do k = 1 , nlev
          do j=1,np
            do i=1,np
     ...
     if (.not. euler_step_dssvar_pack_use_native_impl) then
       call euler_step_dssvar_pack_apply_codon(DSSvar(:,:,1:nlev), elem(ie)%spheremp)
     else
       do k = 1 , nlev
         do j=1,np
            do i=1,np
  ...
  do ie = nets , nete
    ! only perform this operation on thread which owns the first tracer
       ...
       if (.not. euler_step_dssvar_restore_use_native_impl) then
         call euler_step_dssvar_restore_apply_codon(DSSvar(:,:,1:nlev), elem(ie)%rspheremp)
       else
         do k = 1, nlev
           !OMP_COLLAPSE_SIMD
           !DIR_VECTOR_ALIGNED
...
!-----------------------------------------------------------------------------
  ...
  subroutine euler_step_qdp_writeback_apply_codon(qdp, spheremp, qtens, q, np1_qdp)
    use iso_c_binding, only : c_int64_t, c_loc, c_ptr
    implicit none
    real(kind=real_kind), target, intent(inout) :: qdp(np,np,nlev,qsize,2)
    real(kind=real_kind), target, intent(in) :: spheremp(np,np), qtens(np,np,nlev)
    integer, intent(in) :: q, np1_qdp
    interface
       subroutine euler_step_qdp_writeback_codon(np_c, nlev_c, qsize_c, q_c, np1_qdp_c, qdp_p, spheremp_p, qtens_p) &
            bind(c, name='euler_step_qdp_writeback_codon')
         use iso_c_binding, only : c_int64_t, c_ptr
         integer(c_int64_t), value :: np_c, nlev_c, qsize_c, q_c, np1_qdp_c
         type(c_ptr), value :: qdp_p, spheremp_p, qtens_p
       end subroutine euler_step_qdp_writeback_codon
    end interface

    call euler_step_qdp_writeback_codon( &
         int(np, c_int64_t), int(nlev, c_int64_t), int(qsize, c_int64_t), int(q, c_int64_t), int(np1_qdp, c_int64_t), &
         c_loc(qdp), c_loc(spheremp), c_loc(qtens) &
    )
...
!-----------------------------------------------------------------------------
  ...
  subroutine euler_step_dssvar_restore_apply_codon(dssvar, rspheremp)
    use iso_c_binding, only : c_int64_t, c_loc, c_ptr
    implicit none
    real(kind=real_kind), target, intent(inout) :: dssvar(np,np,nlev)
    real(kind=real_kind), target, intent(in) :: rspheremp(np,np)
    interface
       subroutine euler_step_dssvar_restore_codon(np_c, nlev_c, dssvar_p, rspheremp_p) &
            bind(c, name='euler_step_dssvar_restore_codon')
         use iso_c_binding, only : c_int64_t, c_ptr
         integer(c_int64_t), value :: np_c, nlev_c
         type(c_ptr), value :: dssvar_p, rspheremp_p
       end subroutine euler_step_dssvar_restore_codon
    end interface

    call euler_step_dssvar_restore_codon( &
         int(np, c_int64_t), int(nlev, c_int64_t), c_loc(dssvar), c_loc(rspheremp) &
    )
  end subroutine euler_step_dssvar_restore_apply_codon

!-----------------------------------------------------------------------------
!-----------------------------------------------------------------------------

  subroutine euler_step_dssvar_pack_apply_codon(dssvar, spheremp)
    use iso_c_binding, only : c_int64_t, c_loc, c_ptr
    implicit none
    real(kind=real_kind), target, intent(inout) :: dssvar(np,np,nlev)
    real(kind=real_kind), target, intent(in) :: spheremp(np,np)
    interface
       subroutine euler_step_dssvar_pack_codon(np_c, nlev_c, dssvar_p, spheremp_p) &
            bind(c, name='euler_step_dssvar_pack_codon')
         use iso_c_binding, only : c_int64_t, c_ptr
         integer(c_int64_t), value :: np_c, nlev_c
         type(c_ptr), value :: dssvar_p, spheremp_p
       end subroutine euler_step_dssvar_pack_codon
    end interface

    call euler_step_dssvar_pack_codon( &
...
!-----------------------------------------------------------------------------
  ...
  subroutine euler_step_qtens_base_apply_codon(qdp, dp_star, dt, qtens)
    use iso_c_binding, only : c_int64_t, c_loc, c_double, c_ptr
    implicit none
    real(kind=real_kind), target, intent(in) :: qdp(np,np), dp_star(np,np)
    real(kind=real_kind), target, intent(out) :: qtens(np,np)
    real(kind=real_kind), intent(in) :: dt
    interface
       subroutine euler_step_qtens_base_codon(np_c, dt_c, qdp_p, dp_star_p, qtens_p) &
            bind(c, name='euler_step_qtens_base_codon')
         use iso_c_binding, only : c_int64_t, c_double, c_ptr
         integer(c_int64_t), value :: np_c
         real(c_double), value :: dt_c
         type(c_ptr), value :: qdp_p, dp_star_p, qtens_p
       end subroutine euler_step_qtens_base_codon
    end interface

    call euler_step_qtens_base_codon( &
...
!-----------------------------------------------------------------------------
  ...
  subroutine euler_step_qtens_biharmonic_unapply_apply_codon(qtens_biharmonic, spheremp, dp0_in, rhs_viss, dt, nu_q)
    use iso_c_binding, only : c_int64_t, c_loc, c_double, c_ptr
    implicit none
    real(kind=real_kind), target, intent(inout) :: qtens_biharmonic(np,np,nlev)
    real(kind=real_kind), target, intent(in) :: spheremp(np,np), dp0_in(nlev)
    integer, intent(in) :: rhs_viss
    real(kind=real_kind), intent(in) :: dt, nu_q
    interface
       subroutine euler_step_qtens_biharmonic_unapply_codon(np_c, nlev_c, rhs_viss_c, dt_c, nu_q_c, qtens_biharmonic_p, spheremp_p, dp0_p) &
            bind(c, name='euler_step_qtens_biharmonic_unapply_codon')
         use iso_c_binding, only : c_int64_t, c_double, c_ptr
         integer(c_int64_t), value :: np_c, nlev_c, rhs_viss_c
         real(c_double), value :: dt_c, nu_q_c
         type(c_ptr), value :: qtens_biharmonic_p, spheremp_p, dp0_p
       end subroutine euler_step_qtens_biharmonic_unapply_codon
    end interface

    call euler_step_qtens_biharmonic_unapply_codon( &
         int(np, c_int64_t), int(nlev, c_int64_t), int(rhs_viss, c_int64_t), real(dt, c_double), real(nu_q, c_double), &
...
!$omp parallel do private(q,k)
  ...
  interface
     ...
     subroutine vertical_remap_v_scale_codon(np, nlev, v_p, dp_star_p, ttmp_p) &
          bind(c, name='vertical_remap_v_scale_codon')
       use iso_c_binding, only : c_int64_t, c_ptr
       integer(c_int64_t), value :: np, nlev
       type(c_ptr), value :: v_p, dp_star_p, ttmp_p
     end subroutine vertical_remap_v_scale_codon

     subroutine vertical_remap_v_unscale_codon(np, nlev, ttmp_p, dp_p, v_p) &
          bind(c, name='vertical_remap_v_unscale_codon')
       use iso_c_binding, only : c_int64_t, c_ptr
       integer(c_int64_t), value :: np, nlev
...
#undef REMAP_TE
        ...
        if (.not. vertical_remap_v_scale_use_native_impl) then
           call vertical_remap_v_scale_codon( &
                int(np, c_int64_t), int(nlev, c_int64_t), c_loc(elem(ie)%state%v(:,:,:,:,np1)), &
                c_loc(dp_star), c_loc(ttmp) &
           )
        else
...
!         call remap_velocityC(np1,dt,elem,fvm,hvcoord,ie)
  ...
  subroutine vertical_remap_v_scale_select_impl()
    character(len=32) :: impl_name
    integer :: status, n, i, code
    ...
    if (status == 0 .and. n > 0) then
       ...
       vertical_remap_v_scale_use_native_impl = trim(adjustl(impl_name(:n))) == 'native'
    else
       vertical_remap_v_scale_use_native_impl = .false.
    end if
  ...
  subroutine euler_step_qdp_writeback_select_impl()
    character(len=32) :: impl_name
    integer :: status, n, i, code
    ...
    if (status == 0 .and. n > 0) then
      ...
      euler_step_qdp_writeback_use_native_impl = trim(adjustl(impl_name(:n))) == 'native'
    else
      euler_step_qdp_writeback_use_native_impl = .false.
    end if
  ...
  subroutine euler_step_dssvar_restore_select_impl()
    character(len=32) :: impl_name
    integer :: status, n, i, code
    ...
    if (status == 0 .and. n > 0) then
      ...
      euler_step_dssvar_restore_use_native_impl = trim(adjustl(impl_name(:n))) == 'native'
    else
      euler_step_dssvar_restore_use_native_impl = .false.
    end if

    euler_step_dssvar_restore_impl_selected = .true.
  end subroutine euler_step_dssvar_restore_select_impl

  subroutine euler_step_dssvar_pack_select_impl()
    character(len=32) :: impl_name
    integer :: status, n, i, code
    ...
    if (status == 0 .and. n > 0) then
      ...
      euler_step_dssvar_pack_use_native_impl = trim(adjustl(impl_name(:n))) == 'native'
    else
      euler_step_dssvar_pack_use_native_impl = .false.
    end if
  ...
  subroutine euler_step_qtens_base_select_impl()
    ...
    if (status == 0 .and. n > 0) then
      ...
      euler_step_qtens_base_use_native_impl = trim(adjustl(impl_name(:n))) == 'native'
    else
      euler_step_qtens_base_use_native_impl = .false.
    end if
  ...
  subroutine euler_step_qtens_biharmonic_unapply_select_impl()
    ...
    if (status == 0 .and. n > 0) then
      ...
      euler_step_qtens_biharmonic_unapply_use_native_impl = trim(adjustl(impl_name(:n))) == 'native'
    else
      euler_step_qtens_biharmonic_unapply_use_native_impl = .false.
    end if
