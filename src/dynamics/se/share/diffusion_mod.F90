...
!    real(kind=real_kind), dimension(:,:,:,:), pointer :: metinv
    ...
    real(kind=real_kind), dimension(:,:,:,:), pointer :: Dinv2
...
!$omp parallel do private(k,grad_tmp,i,j,v1,v2,gp,vco,div_tmp,zeta_tmp)
       ...
       do k=1,nlev   
          ...
          call gradient(elem(ie)%state%T(:,:,k,np1),deriv,grad_tmp)
          grad_tmp = grad_tmp*rrearth
...
! Map grad_tmp to lat-lon instead of rotating it
          ...
          call divergence(gp,deriv,div_tmp)
          div_tmp=div_tmp*rrearth
        
          call vorticity(vco,deriv,zeta_tmp)
          zeta_tmp=zeta_tmp*rrearth
...
!                grad_T_np1(i,j,2,k,ie) = metdet(i,j)*(metinv(2,1,i,j)*v1 + metinv(2,2,i,j)*v2)
       ...
       (elem(ie)%Dinv(i,j,2,1)*grad_tmp(i,j,1)+elem(ie)%Dinv(i,j,2,2)*grad_tmp(i,j,2))
          ...
          call divergence(grad_T_np1(:,:,:,k,ie),deriv,div_tmp)
          div_tmp=div_tmp*rrearth
          ...
          call gradient(div_np1(:,:,k,ie),deriv,grad_div)
          grad_div=grad_div*rrearth
          call gradient(zeta_np1(:,:,k,ie),deriv,grad_zeta)
          grad_zeta=grad_zeta*rrearth

          do j=1,np
             do i=1,np

                v2 = Dinv2(i,j,1,1)*grad_zeta(i,j,1) + Dinv2(i,j,2,1)*grad_zeta(i,j,2)
                v1 = Dinv2(i,j,1,2)*grad_zeta(i,j,1) + Dinv2(i,j,2,2)*grad_zeta(i,j,2)
                ...
                grad_div(i,j,1) = Dinv2(i,j,1,1)*v1 + Dinv2(i,j,2,1)*v2
                grad_div(i,j,2) = Dinv2(i,j,1,2)*v1 + Dinv2(i,j,2,2)*v2
...
!$omp parallel do private(k,grad_tmp,i,j,v1,v2)
          ...
          do k=1,nlev   
             ...
             call gradient(elem(ie)%state%Q(:,:,k,q),deriv,grad_tmp)
             grad_tmp=grad_tmp*rrearth
...
!$omp parallel do private(k,i,j,div_tmp)
          ...
          do k=1,nlev
             ...
             call divergence(grad_Q_np1(:,:,:,k,q,ie),deriv,div_tmp)
             div_tmp=div_tmp*rrearth
