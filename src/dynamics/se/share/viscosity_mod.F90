...
logical var_coef1
    ...
    interface
       function biharmonic_wk_dp3d_codon(tag) result(tag_out) bind(c, name='biharmonic_wk_dp3d_codon')
         import :: c_int64_t
         integer(c_int64_t), value :: tag
         integer(c_int64_t) :: tag_out
       end function biharmonic_wk_dp3d_codon
    end interface
    ...
    call cam_codon_get_impl('BIHARMONIC_WK_DP3D_IMPL', rt_codon_impl_name, rt_codon_n, rt_codon_status)
    if (.not. (rt_codon_status == 0 .and. rt_codon_n > 0 .and. &
         trim(adjustl(rt_codon_impl_name(:rt_codon_n))) == 'native')) then
       rt_codon_tag_out = biharmonic_wk_dp3d_codon(int(16, c_int64_t))
       if (rt_codon_tag_out /= int(16, c_int64_t)) then
          write(iulog,*) 'biharmonic_wk_dp3d_codon tag roundtrip failed'
          stop 2
       endif
       if (.not. rt_codon_proof_seen) then
          write(iulog,*) 'biharmonic_wk_dp3d implementation = codon'
...
#define IBYP_TEST
    ...
    do ie=nets,nete
       ...
       call curl_sphere(E(:,:,ie),deriv,elem(ie),grade)
       call divergence_sphere(grade,deriv,elem(ie),div)
       call vorticity_sphere(grade,deriv,elem(ie),vor)
       ...
       call gradient_sphere(E(:,:,ie),deriv,elem(ie)%Dinv,grade)
       call vorticity_sphere(grade,deriv,elem(ie),vor)
       call divergence_sphere(grade,deriv,elem(ie),div)
    ...
    do ie=nets,nete
       ...
       call divergence_sphere(gradbig(:,:,:,ie),deriv,elem(ie),divbig(:,:,ie))
    enddo
...
!
  ...
  do ie=nets,nete
     ...
     call divergence_sphere(ulatlon,deriv,elem(ie),phidivu)
     phidivu = phidivu*elem(ie)%spheremp
