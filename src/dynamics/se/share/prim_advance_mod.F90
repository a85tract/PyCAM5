...
contains
  ...
  subroutine prim_advance_init(par,elem,integration)
    ...
    interface
       function prim_advance_init_codon(tag) result(tag_out) bind(c, name='prim_advance_init_codon')
         import :: c_int64_t
         integer(c_int64_t), value :: tag
         integer(c_int64_t) :: tag_out
       end function prim_advance_init_codon
    end interface
    ...
    call cam_codon_get_impl('PRIM_ADVANCE_INIT_IMPL', rt_codon_impl_name, rt_codon_n, rt_codon_status)
    if (.not. (rt_codon_status == 0 .and. rt_codon_n > 0 .and. &
         trim(adjustl(rt_codon_impl_name(:rt_codon_n))) == 'native')) then
       rt_codon_tag_out = prim_advance_init_codon(int(5, c_int64_t))
       if (rt_codon_tag_out /= int(5, c_int64_t)) then
          write(iulog,*) 'prim_advance_init_codon tag roundtrip failed'
          stop 2
       endif
       if (.not. rt_codon_proof_seen) then
          write(iulog,*) 'prim_advance_init implementation = codon'
...
!JMD       call t_barrierf('sync_prim_advance_si', hybrid%par%comm)
       ...
       do ie = nets, nete
          ...
          call gradient(elem(ie)%state%lnps(:,:,n0),deriv,elem(ie)%derived%grad_lnps(:,:,:))
...
!$omp parallel do private(k,i,j)
          ...
          do k=1,nlev
             ...
             call gradient(elem(ie)%state%T(:,:,k,n0),deriv,gradT)
             gradT=gradT*rrearth
             Crkk       = 0.5_real_kind
...
!$omp parallel do private(k,i,j,gVscript,deriv)
          ...
          do k=1,nlev
             ...
             call divergence(gVscript,deriv,div_Vscript(:,:,k))
...
!$omp parallel do private(k,i,j,l)
          ...
          do k=1,nlev
             ...
             call gradient_wk(Gref(:,:,k,ie),deriv,grad_Gref(:,:,:,k))
...
!$omp parallel do private(q,k,i,j,dp)
  ...
  subroutine applycamforcing_dynamics_select_impl()
    ...
    impl_name = 'codon'
    call cam_codon_get_impl('APPLYCAMFORCING_DYNAMICS_IMPL', impl_name, n, status)
...
!     ps => fptr%base(ie)%state%ps_v(:,:,n0)
     ...
     ! vertically eulerian only needs grad(ps)
     if (rsplit==0) &
          call gradient_sphere(fptr%base(ie)%state%ps_v(:,:,n0),deriv,fptr%base(ie)%Dinv,grad_ps)
...
!$omp parallel do private(k,i,j,v1,v2,vtemp)
     ...
     do k=1,nlev
        if (rsplit==0) then
           ...
           call gradient_sphere(p(:,:,k),deriv,fptr%base(ie)%Dinv,grad_p(:,:,:,k))
        endif
...
!                   hvcoord%hybm(k)*(v1*grad_ps(i,j,1) + v2*grad_ps(i,j,2))
        ...
        call divergence_sphere(vtemp,deriv,fptr%base(ie),divdp(:,:,k))
        !vort(:,:,k)=vorticity_sphere(fptr%base(ie)%state%v(:,:,:,k,n0),deriv,fptr%base(ie))
        call vorticity_sphere(fptr%base(ie)%state%v(:,:,:,k,n0),deriv,fptr%base(ie),vort(:,:,k))
...
!$omp parallel do private(k,i,j,v1,v2,E,Ephi,vtemp,vgrad_T,gpterm,glnps1,glnps2)
     ...
     do k=1,nlev
        ...
        call gradient_sphere(fptr%base(ie)%state%T(:,:,k,n0),deriv,fptr%base(ie)%Dinv,vtemp)
        do j=1,np
           do i=1,np
              v1     = fptr%base(ie)%state%v(i,j,1,k,n0)
              v2     = fptr%base(ie)%state%v(i,j,2,k,n0)
        ...
        call gradient_sphere(Ephi(:,:),deriv,fptr%base(ie)%Dinv,vtemp)
...
!              glnps2 = Rgas*gpterm*grad_ps(i,j,2)
     ...
     if (compute_diagnostics) then
        ...
        do k=1,nlev
           ...
           call gradient_sphere(phi(:,:,k),deriv,fptr%base(ie)%Dinv,vtemp)
           do j=1,np
              do i=1,np
                 v1     = fptr%base(ie)%state%v(i,j,1,k,n0)
                 v2     = fptr%base(ie)%state%v(i,j,2,k,n0)
           ...
           call gradient_sphere(fptr%base(ie)%state%phis(:,:),deriv,fptr%base(ie)%Dinv,vtemp)
           do j=1,np
              do i=1,np
                 v1     = fptr%base(ie)%state%v(i,j,1,k,n0)
                 v2     = fptr%base(ie)%state%v(i,j,2,k,n0)
