...
contains
  ...
  subroutine native_mapping_readnl(NLFileName)
    ...
    interface
       function native_mapping_readnl_codon(tag) result(tag_out) bind(c, name='native_mapping_readnl_codon')
         import :: c_int64_t
         integer(c_int64_t), value :: tag
         integer(c_int64_t) :: tag_out
       end function native_mapping_readnl_codon
    end interface
    ...
    call cam_codon_get_impl('NATIVE_MAPPING_READNL_IMPL', rt_codon_impl_name, rt_codon_n, rt_codon_status)
    if (.not. (rt_codon_status == 0 .and. rt_codon_n > 0 .and. &
         trim(adjustl(rt_codon_impl_name(:rt_codon_n))) == 'native')) then
       rt_codon_tag_out = native_mapping_readnl_codon(int(30, c_int64_t))
       if (rt_codon_tag_out /= int(30, c_int64_t)) then
          write(iulog,*) 'native_mapping_readnl_codon tag roundtrip failed'
          stop 2
       endif
       if (.not. rt_codon_proof_seen) then
          write(iulog,*) 'native_mapping_readnl implementation = codon'
