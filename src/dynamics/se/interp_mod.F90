...
#define SE_MISC_LABEL 'setup_history_interpolation'
    interface
       function setup_history_interpolation_codon(tag) result(tag_out) bind(c, name='setup_history_interpolation_codon')
         import :: c_int64_t
         integer(c_int64_t), value :: tag
         integer(c_int64_t) :: tag_out
       end function setup_history_interpolation_codon
    end interface
    ...
    if (.not. rt_codon_proof_seen .and. &
       ...
       rt_codon_tag_out = setup_history_interpolation_codon(int(SE_MISC_TAG, c_int64_t))
