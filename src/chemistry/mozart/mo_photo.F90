  ...
  subroutine photo_inti( xs_coef_file, xs_short_file, xs_long_file, rsf_file, &
    ...
    is_xactive : if( xactive_prates ) then
       ...
       if (do_jeuv) then
          ...
          call addfld('Qbkgndtot','cm-3 sec-1', pver, 'A', 'background ionization rate ', phys_decomp )
          call addfld('Qbkgnd_o1','cm-3 sec-1', pver, 'A', 'background ionization rate ', phys_decomp )
          call addfld('Qbkgnd_o2','cm-3 sec-1', pver, 'A', 'background ionization rate ', phys_decomp )
          call addfld('Qbkgnd_n2','cm-3 sec-1', pver, 'A', 'background ionization rate ', phys_decomp )
          call addfld('Qbkgnd_n1','cm-3 sec-1', pver, 'A', 'background ionization rate ', phys_decomp )
          call addfld('Qbkgnd_no','cm-3 sec-1', pver, 'A', 'background ionization rate ', phys_decomp )
       endif
       if (do_jshort) then
...
!-----------------------------------------------------------------
    ...
    interface
       ...
       subroutine table_photo_stage_dispatch_codon(stage_c, ncol_c, pcols_c, pver_c, ncol_abs_c, nfs_c, gas_pcnst_c, &
            ...
            zen_angle_c, srf_alb_c, rgrav_c, esfact_c, pmid_p, pdel_p, col_dens_p, lwc_p, clouds_p, &
            temper_p, zmid_p, zint_p, vmr_p, invariants_p, parg_p, colo3_p, fac1_p, lwc_line_p, &
         ...
         integer(c_int64_t), value :: o_is_inv_c, o2_is_inv_c, o3_is_inv_c, n2_is_inv_c, no_is_inv_c
         integer(c_int64_t), value :: o_ndx_c, o2_ndx_c, o3_inv_ndx_c, o3_ndx_c, n2_ndx_c, no_ndx_c, indexm_c
         integer(c_int64_t), value :: jno_ndx_c, jho2no2_ndx_c, has_o2_col_c, has_o3_col_c
         real(c_double), value :: pa2mb_c, zen_angle_c, srf_alb_c, rgrav_c, esfact_c
         type(c_ptr), value :: pmid_p, pdel_p, col_dens_p, lwc_p, clouds_p, temper_p, zmid_p, zint_p
         type(c_ptr), value :: vmr_p, invariants_p, parg_p, colo3_p, fac1_p, lwc_line_p, cld_line_p, tline_p, zarg_p
         type(c_ptr), value :: o_den_p, o2_den_p, o3_den_p, no_den_p, n2_den_p
       ...
       daylight : if( sza >= 0._r8 .and. sza < max_zen_angle ) then
          if (table_photo_use_native_impl) then
             ...
             call table_photo_stage_dispatch_codon( &
                  ...
                  int(jno_ndx, c_int64_t), int(jho2no2_ndx, c_int64_t), &
                  ...
                  real(rgrav_cloud, c_double), real(esfact, c_double), c_loc(pmid), c_loc(pdel), c_loc(col_dens), c_loc(lwc), &
