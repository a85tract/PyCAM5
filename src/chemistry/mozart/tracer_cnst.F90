...
!-------------------------------------------------------------------
  subroutine tracer_cnst_setopts(       &
    ...
    interface
       function tracer_cnst_setopts_codon() result(out_c) bind(c, name="tracer_cnst_setopts_codon")
         use iso_c_binding, only : c_int64_t
         integer(c_int64_t) :: out_c
       end function tracer_cnst_setopts_codon
    end interface

    codon_entry = tracer_cnst_setopts_codon()
...
!-------------------------------------------------------------------
  subroutine tracer_cnst_defaultopts(   &
    ...
    character(len=32) :: impl_name
    logical :: use_native_impl
    logical, save :: tracer_cnst_defaultopts_logged = .false.
    ...
    interface
       subroutine tracer_cnst_defaultopts_codon(file_len_c, file_p, filelist_len_c, filelist_p, &
            ...
            bind(c, name="tracer_cnst_defaultopts_codon")
       ...
       end subroutine tracer_cnst_defaultopts_codon
    end interface

    impl_name = 'codon'
    call cam_codon_get_impl('TRACER_CNST_DEFAULTOPTS_IMPL', impl_name, n, status)
    if (status == 0 .and. n > 0) then
       do i = 1, n
    ...
    if (.not. use_native_impl) then
       ...
       call tracer_cnst_defaultopts_codon( &
            ...
            merge(1_c_int64_t, 0_c_int64_t, present(tracer_cnst_file_out)), c_loc(filename_out_ascii(1)), &
            merge(1_c_int64_t, 0_c_int64_t, present(tracer_cnst_filelist_out)), c_loc(filelist_out_ascii(1)), &
            merge(1_c_int64_t, 0_c_int64_t, present(tracer_cnst_datapath_out)), c_loc(datapath_out_ascii(1)), &
            merge(1_c_int64_t, 0_c_int64_t, present(tracer_cnst_type_out)), c_loc(data_type_out_ascii(1)), &
            merge(1_c_int64_t, 0_c_int64_t, present(tracer_cnst_specifier_out)), c_loc(specifier_out_ascii(1,1)), &
            merge(1_c_int64_t, 0_c_int64_t, present(tracer_cnst_rmfile_out)), &
            merge(1_c_int64_t, 0_c_int64_t, present(tracer_cnst_cycle_yr_out)), &
            merge(1_c_int64_t, 0_c_int64_t, present(tracer_cnst_fixed_ymd_out)), &
            merge(1_c_int64_t, 0_c_int64_t, present(tracer_cnst_fixed_tod_out)), c_loc(scalar_out(1)) &
       )

       if ( present(tracer_cnst_file_out) ) then
          tracer_cnst_file_out = ' '
          do i = 1, min(len(tracer_cnst_file_out), len(filename))
             tracer_cnst_file_out(i:i) = achar(int(filename_out_ascii(i)))
          end do
       end if
       if ( present(tracer_cnst_filelist_out) ) then
          tracer_cnst_filelist_out = ' '
          do i = 1, min(len(tracer_cnst_filelist_out), len(filelist))
             tracer_cnst_filelist_out(i:i) = achar(int(filelist_out_ascii(i)))
          end do
       end if
       if ( present(tracer_cnst_datapath_out) ) then
          tracer_cnst_datapath_out = ' '
          do i = 1, min(len(tracer_cnst_datapath_out), len(datapath))
             tracer_cnst_datapath_out(i:i) = achar(int(datapath_out_ascii(i)))
          end do
       end if
       if ( present(tracer_cnst_type_out) ) then
          tracer_cnst_type_out = ' '
          do i = 1, min(len(tracer_cnst_type_out), len(data_type))
             tracer_cnst_type_out(i:i) = achar(int(data_type_out_ascii(i)))
          end do
       end if
       if ( present(tracer_cnst_specifier_out) ) then
          tracer_cnst_specifier_out = ' '
          do j = 1, min(size(tracer_cnst_specifier_out), size(specifier))
             do i = 1, min(len(tracer_cnst_specifier_out), len(specifier))
                tracer_cnst_specifier_out(j)(i:i) = achar(int(specifier_out_ascii(i,j)))
             end do
          end do
       end if
       if ( present(tracer_cnst_rmfile_out) ) tracer_cnst_rmfile_out = scalar_out(1) /= 0_c_int64_t
       if ( present(tracer_cnst_cycle_yr_out) ) tracer_cnst_cycle_yr_out = int(scalar_out(2))
       if ( present(tracer_cnst_fixed_ymd_out) ) tracer_cnst_fixed_ymd_out = int(scalar_out(3))
       if ( present(tracer_cnst_fixed_tod_out) ) tracer_cnst_fixed_tod_out = int(scalar_out(4))

       if (masterproc .and. .not. tracer_cnst_defaultopts_logged) then
          write(iulog,'(A)') 'tracer_cnst_defaultopts implementation = codon'
          tracer_cnst_defaultopts_logged = .true.
          call flush(iulog)
       end if
       return
    end if
...
!-------------------------------------------------------------------
  subroutine tracer_cnst_adv( pbuf2d, state )
    ...
    interface
       function tracer_cnst_adv_codon(active) result(out_c) bind(c, name="tracer_cnst_adv_codon")
         use iso_c_binding, only : c_int64_t
         integer(c_int64_t), value :: active
         integer(c_int64_t) :: out_c
       end function tracer_cnst_adv_codon
    end interface

    impl_name = 'codon'
    call cam_codon_get_impl('TRACER_CNST_ADV_IMPL', impl_name, n, status)
    if (status == 0 .and. n > 0) then
       do i = 1, n
          code = iachar(impl_name(i:i))
          if (code >= iachar('A') .and. code <= iachar('Z')) impl_name(i:i) = achar(code + iachar('a') - iachar('A'))
    ...
    if (.not. use_native_impl) then
       active_c = tracer_cnst_adv_codon(merge(1_c_int64_t, 0_c_int64_t, num_tracer_cnst >= 1))
       if (active_c == 0_c_int64_t) then
          if (masterproc) then
             write(iulog,'(A)') 'tracer_cnst_adv direct = codon no-tracer-cnst no-op'
             call flush(iulog)
          end if
          return
       end if
...
!-------------------------------------------------------------------
  subroutine get_cnst_data( field_name, data, ncol, lchnk, pbuf  )
    ...
    interface
       function get_cnst_data_codon(active) result(out_c) bind(c, name="get_cnst_data_codon")
         use iso_c_binding, only : c_int64_t
         integer(c_int64_t), value :: active
         integer(c_int64_t) :: out_c
       end function get_cnst_data_codon
    end interface
    ...
    impl_name = 'codon'
    call cam_codon_get_impl('GET_CNST_DATA_IMPL', impl_name, n, status)
    if (status == 0 .and. n > 0) then
       do i = 1, n
          code = iachar(impl_name(i:i))
          if (code >= iachar('A') .and. code <= iachar('Z')) impl_name(i:i) = achar(code + iachar('a') - iachar('A'))
    ...
    if (.not. use_native_impl) then
       active_c = get_cnst_data_codon(merge(1_c_int64_t, 0_c_int64_t, num_tracer_cnst >= 1))
       if (active_c == 0_c_int64_t) then
          if (masterproc) then
             write(iulog,'(A)') 'get_cnst_data direct = codon no-tracer-cnst no-op'
             call flush(iulog)
          end if
          return
       end if
