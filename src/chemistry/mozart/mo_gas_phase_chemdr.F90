module mo_gas_phase_chemdr
  ...
  use gas_phase_chemdr_scheme, only : gas_phase_chemdr_run,      &
       gas_phase_set_thermal_rates_run, gas_phase_adjust_rates_run, &
       gas_phase_adjust_photolysis_run
  use ap_chemistry_common_kernels, only : chemistry_map_constituents, &
       chemistry_set_mean_mass, chemistry_mmr_to_vmr,               &
       chemistry_vmr_to_mmr, chemistry_mmr_to_vmri,                 &
       chemistry_h2o_to_vmr, chemistry_form_tendencies,             &
       chemistry_map_surface_flux, chemistry_clip_negative
  ...
  integer :: synoz_ndx, so4_ndx, h2o_ndx, o2_ndx, o_ndx, h_ndx, n_ndx
  integer :: hno3_ndx, hcl_ndx, dst_ndx, cldice_ndx
...
!! \htmlinclude gas_phase_chemdr_run.html
  subroutine gas_phase_chemdr_host_run(lchnk, ncol, pcols, pver, pverp, pcnst, &
                              imozart, q, &
                              phis, zm, zi, calday, &
                              tfld, pmid, pdel, pint,  &
                              cldw, troplev, &
    ...
    use chem_mods,         only : nabscol, nfs, indexm, clscnt1, &
         clscnt4, nzcnt, clsmap, permute, cls_rxt_cnt, adv_mass
    use cam_abortutils,    only : endrun
    use cam_logfile,       only : iulog
    use mo_tracname,       only : solsym
    use physconst,         only : rga, mwdry
    use mo_photo,          only : set_ub_col, setcol, table_photo, xactive_photo
    use mo_exp_sol,        only : exp_sol_scheme_entry_count
    use mo_imp_sol,        only : itermax, epsilon, factor, small, &
         imp_sol_scheme_entry_count
...
!
    ...
    integer,        intent(in)    :: pcols, pver, pverp, pcnst      ! declared dimensions
    ...
    real(r8),       intent(in)    :: zi(pcols,pverp)                 ! interface geopotential height above the surface (m)
    real(r8),       intent(in)    :: pint(pcols,pverp)               ! interface pressures (Pa)
    ...
    real(r8) :: delta
    real(r8) :: solver_prod(ncol,pver,max(1,clscnt4))
    real(r8) :: solver_loss(ncol,pver,max(1,clscnt4))
    real(r8) :: prod_hydrogen_peroxides(ncol,pver)
    real(r8) :: last_failure_dt
    integer :: solver_failure_count, solver_errflg, mechanism_ierr
    integer :: mean_mass_ierr
    logical :: fixed_mbar
    character(len=512) :: solver_errmsg
...
!
    ...
    !-----------------------------------------------------------------------
    call chemistry_map_constituents(ncol, pcols, pver, pcnst,     &
    ...
    !-----------------------------------------------------------------------
    fixed_mbar = .not. (waccmx_is('ionosphere') .or.             &
         waccmx_is('neutral'))
    call chemistry_set_mean_mass(ncol, pver, gas_pcnst,          &
         fixed_mbar, mwdry, o2_ndx, o_ndx, h_ndx, n_ndx,        &
         adv_mass, mmr(:ncol,:,:), mbar, mean_mass_ierr)
    if (mean_mass_ierr /= 0) call endrun(                        &
         'gas_phase_chemdr: not able to compute mean mass')

    !-----------------------------------------------------------------------
    !        ... Xform from mmr to vmr
    !-----------------------------------------------------------------------
    call chemistry_mmr_to_vmr(ncol, pver, gas_pcnst, adv_mass,  &
         mmr(:ncol,:,:), mbar, vmr)
...
!
    ...
    if (h2o_ndx>0) then
       ...
       !-----------------------------------------------------------------------
       call chemistry_h2o_to_vmr(ncol, pver, 18._r8,             &
            q(:ncol,:,1), mbar, h2ovmr)
    ...
    stratochem: if ( has_strato_chem ) then
       ...
       call chemistry_mmr_to_vmri(ncol, pver, cnst_mw(cldice_ndx), &
            cldice(:ncol,:), mbar, h2o_cond)
...
!            ratecon_sfstrat needs total hcl.
    ...
    !-----------------------------------------------------------------------
    call gas_phase_set_thermal_rates_run(ncol, pver, rxntot,     &
         tfld(:ncol,:), reaction_rates, mechanism_ierr)
    if (mechanism_ierr /= 0) call endrun(                        &
         'gas_phase_chemdr: pp_trop_mam3 thermal-rate dimensions')
    ...
    call gas_phase_adjust_rates_run(ncol, pver, nfs, rxntot,     &
         reaction_rates, invariants, invariants(:,:,indexm),     &
         mechanism_ierr)
    if (mechanism_ierr /= 0) call endrun(                        &
         'gas_phase_chemdr: pp_trop_mam3 rate-adjust dimensions')
    ...
    call gas_phase_adjust_photolysis_run(ncol, pver, nfs, phtcnt, &
         reaction_rates(:,:,1:phtcnt), invariants,               &
         invariants(:,:,indexm))
...
!
    ...
    call gas_phase_chemdr_run(1, ncol, pver, gas_pcnst, rxntot, &
         extcnt, clscnt1, clscnt4, nzcnt, itermax, o3_ndx,     &
         o3s_ndx, ndx_h2so4, delt, invariants(:,:,indexm),    &
         ltrop_sol(:ncol), clsmap(1:clscnt4,4),               &
         permute(1:clscnt4,4), cls_rxt_cnt(1,4), epsilon,     &
         factor, small, reaction_rates, het_rates, extfrc,    &
         vmr, o3s_loss, del_h2so4_gasprod, solver_prod,       &
         solver_loss, exp_sol_scheme_entry_count,             &
         imp_sol_scheme_entry_count, solver_failure_count,    &
         last_failure_dt, solver_errmsg, solver_errflg)
    call t_stopf('ap_exp_sol_run')
    if (solver_errflg /= 0) then
       write(iulog,*) trim(solver_errmsg)
       call endrun('gas_phase_chemdr explicit solver error')
    end if
    ...
    call gas_phase_chemdr_run(2, ncol, pver, gas_pcnst, rxntot, &
         extcnt, clscnt1, clscnt4, nzcnt, itermax, o3_ndx,     &
         o3s_ndx, ndx_h2so4, delt, invariants(:,:,indexm),    &
         ltrop_sol(:ncol), clsmap(1:clscnt4,4),               &
         permute(1:clscnt4,4), cls_rxt_cnt(1,4), epsilon,     &
    ...
    if (solver_errflg /= 0) then
       write(iulog,*) trim(solver_errmsg)
       call endrun('gas_phase_chemdr implicit solver error')
    end if
    if (solver_failure_count > 0) then
       write(iulog,'('' gas_phase_chemdr: decoupled solver recorded '',i6, &
            '' failed Newton steps; last dt = '',1p,e21.13)') &
            solver_failure_count, last_failure_dt
    end if

    prod_hydrogen_peroxides = 0._r8
    do i = 1, clscnt4
       j = clsmap(i,4)
       call outfld(trim(solsym(j))//'_CHMP', solver_prod(:,:,i), ncol, lchnk)
       call outfld(trim(solsym(j))//'_CHML', solver_loss(:,:,i), ncol, lchnk)
       if (trim(solsym(j)) == 'ALKOOH' .or. &
           trim(solsym(j)) == 'C2H5OOH' .or. &
           trim(solsym(j)) == 'CH3OOH' .or. &
           trim(solsym(j)) == 'CH3COOH' .or. &
           trim(solsym(j)) == 'CH3COOOH' .or. &
           trim(solsym(j)) == 'C3H7OOH' .or. &
           trim(solsym(j)) == 'EOOH' .or. &
           trim(solsym(j)) == 'ISOPOOH' .or. &
           trim(solsym(j)) == 'MACROOH' .or. &
           trim(solsym(j)) == 'MEKOOH' .or. &
           trim(solsym(j)) == 'POOH' .or. &
           trim(solsym(j)) == 'ROOH' .or. &
           trim(solsym(j)) == 'TERPOOH' .or. &
           trim(solsym(j)) == 'TOLOOH' .or. &
           trim(solsym(j)) == 'XOOH') then
          prod_hydrogen_peroxides = prod_hydrogen_peroxides + &
               solver_prod(:,:,i)
       end if
    end do
    call outfld('H_PEROX_CHMP', prod_hydrogen_peroxides, ncol, lchnk)

    if( h2o_ndx>0) call outfld( 'H2O_GAS',  vmr(1,1,h2o_ndx),  ncol ,lchnk )

!
! jfl : CCMI : O3S update is performed inside gas_phase_chemdr_run.
!
...
!
    ...
    !-----------------------------------------------------------------------
    call chemistry_clip_negative(ncol, pver, gas_pcnst, vmr)
    ...
    !-----------------------------------------------------------------------
    call chemistry_vmr_to_mmr(ncol, pver, gas_pcnst, adv_mass,   &
         vmr, mbar, mmr_tend(:ncol,:,:))
    ...
    !         ... Form the tendencies
    !-----------------------------------------------------------------------
    mmr_new(:ncol,:,:) = mmr_tend(:ncol,:,:)
    call chemistry_form_tendencies(ncol, pcols, pver, pcnst,     &
         gas_pcnst, map2chm, delt_inverse, mmr, mmr_new,         &
         mmr_tend, qtend)
    ...
    call chemistry_map_surface_flux(ncol, pcols, pcnst, gas_pcnst, &
         map2chm, sflx, cflx, drydepflx)
...
!
  ...
  end subroutine gas_phase_chemdr_host_run
  ...
  subroutine gas_phase_chemdr(lchnk, ncol, imozart, q, &
    ...
    call gas_phase_chemdr_host_run(lchnk, ncol, pcols, pver, pverp, pcnst, &
