module dust_sediment_mod
  ...
  use ap_dust_sediment_tend_scheme, only: dust_sediment_vel, &
       dust_sediment_tend_run_core => dust_sediment_tend_run
...
contains
  ...
  subroutine dust_sediment_tend(ncol, dtime, pint, pmid, pdel, t, &
    ...
    real(r8), intent(in) :: dustmr(pcols,pver)
    real(r8), intent(in) :: pvdust(pcols,pverp)
    real(r8), intent(out) :: dusttend(pcols,pver)
    real(r8), intent(out) :: sfdust(pcols)
    character(len=512) :: errmsg
    integer :: errflg

    call t_startf('ap_dust_sediment_tend_run')
    call dust_sediment_tend_run_core(ncol, dtime, pint, pmid, pdel, t, &
