module ap_sslt_sections_private
  ...
  public :: ap_sslt_sections_init
  public :: ap_sslt_section_fluxes
  public :: nsections
  public :: Dg
  public :: rdry
  ...
  real(r8) :: bm(nsections), rdry(nsections), rm(nsections)
  real(r8) :: consta(4,nsections), constb(4,nsections)

contains


  subroutine ap_sslt_sections_init()
    integer :: m

    rdry(:) = Dg(:)/2._r8
    rm(:) = 1.814_r8*rdry(:)*1.e6_r8
    bm(:) = (0.380_r8-log10(rm(:)))/0.65_r8
    ...
    do m = 1,nsections
       ...
       elseif (m .ge. 22 .and. m .le. 40) then
          consta(4,m) = (1.373_r8*rm(m)**(-3)*(1+0.057_r8*rm(m)**1.05_r8) &
               * 10**(1.19_r8*exp(-bm(m)**2))) * (rm(m)-rm(m-1))
       endif
    end do
  end subroutine ap_sslt_sections_init


  function ap_sslt_section_fluxes(sst, u10cubed, ncol) result(fi)
    ...
    W(:ncol) = 3.84e-6_r8*u10cubed(:ncol)*0.1_r8
  ...
  end function ap_sslt_section_fluxes

end module ap_sslt_sections_private
