...
!===============================================================================
  ...
  subroutine compute_uwshcu_inv_run( mix      , mkx        , iend          , ncnst     , dt       ,  &
    ...
    real(r8), intent(in)    :: ps0_inv(:,:)       !  Environmental pressure at the interfaces [ Pa ]
    real(r8), intent(in)    :: zs0_inv(:,:)       !  Environmental height at the interfaces   [ m ]
    real(r8), intent(in)    :: p0_inv(:,:)          !  Environmental pressure at the layer mid-point [ Pa ]
    real(r8), intent(in)    :: z0_inv(:,:)          !  Environmental height at the layer mid-point [ m ]
    real(r8), intent(in)    :: dp0_inv(:,:)         !  Environmental layer pressure thickness [ Pa ] > 0.
    real(r8), intent(in)    :: dpdry0_inv(:,:)      !  Environmental dry layer pressure thickness [ Pa ]
    real(r8), intent(in)    :: u0_inv(:,:)          !  Environmental zonal wind [ m/s ]
    real(r8), intent(in)    :: v0_inv(:,:)          !  Environmental meridional wind [ m/s ]
    real(r8), intent(in)    :: qv0_inv(:,:)         !  Environmental water vapor specific humidity [ kg/kg ]
    real(r8), intent(in)    :: ql0_inv(:,:)         !  Environmental liquid water specific humidity [ kg/kg ]
    real(r8), intent(in)    :: qi0_inv(:,:)         !  Environmental ice specific humidity [ kg/kg ]
    real(r8), intent(in)    :: t0_inv(:,:)          !  Environmental temperature [ K ]
    real(r8), intent(in)    :: s0_inv(:,:)          !  Environmental dry static energy [ J/kg ]
    real(r8), intent(in)    :: tr0_inv(:,:,:)   !  Environmental tracers [ #, kg/kg ]
    real(r8), intent(in)    :: tke_inv(:,:)       !  Turbulent kinetic energy at the interfaces [ m2/s2 ]
    real(r8), intent(in)    :: cldfrct_inv(:,:)     !  Total cloud fraction at the previous time step [ fraction ]
    real(r8), intent(in)    :: concldfrct_inv(:,:)  !  Total convective ( shallow + deep ) cloud fraction
                                                        !  at the previous time step [ fraction ]
    real(r8), intent(in)    :: pblh(:)                !  Height of PBL [ m ]
    real(r8), intent(inout) :: cush(:)                !  Convective scale height [ m ]
    real(r8), intent(out)   :: umf_inv(:,:)       !  Updraft mass flux at the interfaces [ kg/m2/s ]
    real(r8), intent(out)   :: qvten_inv(:,:)       !  Tendency of water vapor specific humidity [ kg/kg/s ]
    real(r8), intent(out)   :: qlten_inv(:,:)       !  Tendency of liquid water specific humidity [ kg/kg/s ]
    real(r8), intent(out)   :: qiten_inv(:,:)       !  Tendency of ice specific humidity [ kg/kg/s ]
    real(r8), intent(out)   :: sten_inv(:,:)        !  Tendency of dry static energy [ J/kg/s ]
    real(r8), intent(out)   :: uten_inv(:,:)        !  Tendency of zonal wind [ m/s2 ]
    real(r8), intent(out)   :: vten_inv(:,:)        !  Tendency of meridional wind [ m/s2 ]
    real(r8), intent(out)   :: trten_inv(:,:,:) !  Tendency of tracers [ #/s, kg/kg/s ]
    real(r8), intent(out)   :: qrten_inv(:,:)       !  Tendency of rain water specific humidity [ kg/kg/s ]
    real(r8), intent(out)   :: qsten_inv(:,:)       !  Tendency of snow specific humidity [ kg/kg/s ]
    real(r8), intent(out)   :: precip(:)              !  Precipitation ( rain + snow ) flux at the surface [ m/s ]
    real(r8), intent(out)   :: snow(:)                !  Snow flux at the surface [ m/s ]
    real(r8), intent(out)   :: evapc_inv(:,:)       !  Evaporation of precipitation [ kg/kg/s ]
    real(r8), intent(out)   :: rliq(:)                !  Vertical integral of tendency of detrained cloud condensate qc [ m/s ]
    real(r8), intent(out)   :: slflx_inv(:,:)     !  Updraft liquid static energy flux [ J/kg * kg/m2/s ]
    real(r8), intent(out)   :: qtflx_inv(:,:)     !  Updraft total water flux [ kg/kg * kg/m2/s ]
    real(r8), intent(out)   :: flxprc1_inv(:,:)   ! uw grid-box mean rain+snow flux (kg m^-2 s^-1)
                                                        ! for physics buffer calls in convect_shallow.F90
    real(r8), intent(out)   :: flxsnow1_inv(:,:)  ! uw grid-box mean snow flux (kg m^-2 s^-1)
                                                        ! for physics buffer calls in convect_shallow.F90

    real(r8), intent(out)   :: cufrc_inv(:,:)       !  Shallow cumulus cloud fraction at the layer mid-point [ fraction ]
    real(r8), intent(out)   :: qcu_inv(:,:)         !  Liquid+ice specific humidity within cumulus updraft [ kg/kg ]
    real(r8), intent(out)   :: qlu_inv(:,:)         !  Liquid water specific humidity within cumulus updraft [ kg/kg ]
    real(r8), intent(out)   :: qiu_inv(:,:)         !  Ice specific humidity within cumulus updraft [ kg/kg ]
    real(r8), intent(out)   :: qc_inv(:,:)          !  Tendency of cumulus condensate detrained into the environment [ kg/kg/s ]
    real(r8), intent(out)   :: cbmf(:)                !  Cumulus base mass flux [ kg/m2/s ]
    real(r8), intent(out)   :: cnt_inv(:)             !  Cumulus top  interface index, cnt = kpen [ no ]
    real(r8), intent(out)   :: cnb_inv(:)             !  Cumulus base interface index, cnb = krel - 1 [ no ]
...
!     end do
  ...
  subroutine compute_uwshcu_run( mix      , mkx       , iend         , ncnst    , dt        , &
    ...
    integer , intent(in)    :: ncnst
    real(r8), intent(in)    :: dt                             !  Time step : 2*delta_t [ s ]
    real(r8), intent(in)    :: ps0_in(:,0:)              !  Environmental pressure at the interfaces [ Pa ]
    real(r8), intent(in)    :: zs0_in(:,0:)              !  Environmental height at the interfaces [ m ]
    real(r8), intent(in)    :: p0_in(:,:)                 !  Environmental pressure at the layer mid-point [ Pa ]
    real(r8), intent(in)    :: z0_in(:,:)                 !  Environmental height at the layer mid-point [ m ]
    real(r8), intent(in)    :: dp0_in(:,:)                !  Environmental layer pressure thickness [ Pa ] > 0.
    real(r8), intent(in)    :: dpdry0_in(:,:)             !  Environmental dry layer pressure thickness [ Pa ]
    real(r8), intent(in)    :: u0_in(:,:)                 !  Environmental zonal wind [ m/s ]
    real(r8), intent(in)    :: v0_in(:,:)                 !  Environmental meridional wind [ m/s ]
    real(r8), intent(in)    :: qv0_in(:,:)                !  Environmental water vapor specific humidity [ kg/kg ]
    real(r8), intent(in)    :: ql0_in(:,:)                !  Environmental liquid water specific humidity [ kg/kg ]
    real(r8), intent(in)    :: qi0_in(:,:)                !  Environmental ice specific humidity [ kg/kg ]
    real(r8), intent(in)    :: t0_in(:,:)                 !  Environmental temperature [ K ]
    real(r8), intent(in)    :: s0_in(:,:)                 !  Environmental dry static energy [ J/kg ]
    real(r8), intent(in)    :: tr0_in(:,:,:)          !  Environmental tracers [ #, kg/kg ]
    real(r8), intent(in)    :: tke_in(:,0:)              !  Turbulent kinetic energy at the interfaces [ m2/s2 ]
    real(r8), intent(in)    :: cldfrct_in(:,:)            !  Total cloud fraction at the previous time step [ fraction ]
    real(r8), intent(in)    :: concldfrct_in(:,:)         !  Total convective cloud fraction
                                                              ! at the previous time step [ fraction ]
    real(r8), intent(in)    :: pblh_in(:)                   !  Height of PBL [ m ]
    real(r8), intent(inout) :: cush_inout(:)                !  Convective scale height [ m ]

    real(r8)                   tw0_in(mix,mkx)                !  Wet bulb temperature [ K ]
    real(r8)                   qw0_in(mix,mkx)                !  Wet-bulb specific humidity [ kg/kg ]

    real(r8), intent(out)   :: umf_out(:,0:)             !  Updraft mass flux at the interfaces [ kg/m2/s ]
    real(r8), intent(out)   :: qvten_out(:,:)             !  Tendency of water vapor specific humidity [ kg/kg/s ]
    real(r8), intent(out)   :: qlten_out(:,:)             !  Tendency of liquid water specific humidity [ kg/kg/s ]
    real(r8), intent(out)   :: qiten_out(:,:)             !  Tendency of ice specific humidity [ kg/kg/s ]
    real(r8), intent(out)   :: sten_out(:,:)              !  Tendency of dry static energy [ J/kg/s ]
    real(r8), intent(out)   :: uten_out(:,:)              !  Tendency of zonal wind [ m/s2 ]
    real(r8), intent(out)   :: vten_out(:,:)              !  Tendency of meridional wind [ m/s2 ]
    real(r8), intent(out)   :: trten_out(:,:,:)       !  Tendency of tracers [ #/s, kg/kg/s ]
    real(r8), intent(out)   :: qrten_out(:,:)             !  Tendency of rain water specific humidity [ kg/kg/s ]
    real(r8), intent(out)   :: qsten_out(:,:)             !  Tendency of snow specific humidity [ kg/kg/s ]
    real(r8), intent(out)   :: precip_out(:)                !  Precipitation ( rain + snow ) rate at surface [ m/s ]
    real(r8), intent(out)   :: snow_out(:)                  !  Snow rate at surface [ m/s ]
    real(r8), intent(out)   :: evapc_out(:,:)             !  Tendency of evaporation of precipitation [ kg/kg/s ]
    real(r8), intent(out)   :: slflx_out(:,0:)           !  Updraft/pen.entrainment liquid static energy flux
                                                              ! [ J/kg * kg/m2/s ]
    real(r8), intent(out)   :: qtflx_out(:,0:)           !  updraft/pen.entrainment total water flux [ kg/kg * kg/m2/s ]
    real(r8), intent(out)   :: flxprc1_out(:,0:)         ! precip (rain+snow) flux
    real(r8), intent(out)   :: flxsnow1_out(:,0:)        ! snow flux
    real(r8), intent(out)   :: cufrc_out(:,:)             !  Shallow cumulus cloud fraction at the layer mid-point [ fraction ]
    real(r8), intent(out)   :: qcu_out(:,:)               !  Condensate water specific humidity within cumulus updraft [ kg/kg ]
    real(r8), intent(out)   :: qlu_out(:,:)               !  Liquid water specific humidity within cumulus updraft [ kg/kg ]
    real(r8), intent(out)   :: qiu_out(:,:)               !  Ice specific humidity within cumulus updraft [ kg/kg ]
    real(r8), intent(out)   :: cbmf_out(:)                  !  Cloud base mass flux [ kg/m2/s ]
    real(r8), intent(out)   :: qc_out(:,:)                !  Tendency of detrained cumulus condensate
                                                              ! into the environment [ kg/kg/s ]
    real(r8), intent(out)   :: rliq_out(:)                  !  Vertical integral of qc_out [ m/s ]
    real(r8), intent(out)   :: cnt_out(:)                   !  Cumulus top  interface index, cnt = kpen [ no ]
    real(r8), intent(out)   :: cnb_out(:)                   !  Cumulus base interface index, cnb = krel - 1 [ no ]
...
!the indices are not reversed, these variables go into compute_mcshallow_inv, this is why they are called "flxprc1" and "flxsnow1".
 ...
 20 return
  ...
  end module ap_uwshcu_processes_scheme
