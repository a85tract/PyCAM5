...
subroutine rad_rrtmg_sw_run(pcols, pver, pverp, lchnk, ncol, rrtmg_levs, cpair, &
   ...
   real(r8), intent(in) :: E_aer_tau(:,:,:)
   real(r8), intent(in) :: E_aer_tau_w(:,:,:)
   real(r8), intent(in) :: E_aer_tau_w_g(:,:,:)
   real(r8), intent(in) :: E_aer_tau_w_f(:,:,:)
   real(r8), intent(in) :: eccf, E_coszrs(:), sfac(:)
   real(r8), intent(in) :: E_asdir(:), E_asdif(:)
   real(r8), intent(in) :: E_aldir(:), E_aldif(:)

   real(r8), intent(out) :: solin(:), qrs(:,:)
   real(r8), intent(out) :: qrsc(:,:)
   real(r8), intent(out) :: fsnt(:), fsntc(:)
   real(r8), intent(out) :: fsntoa(:), fsutoa(:)
   real(r8), intent(out) :: fsntoac(:), fsnirtoa(:)
   real(r8), intent(out) :: fsnrtoac(:), fsnrtoaq(:)
   real(r8), intent(out) :: fsns(:), fsnsc(:)
   real(r8), intent(out) :: fsdsc(:), fsds(:)
   real(r8), intent(out) :: sols(:), soll(:)
   ...
   real(r8), optional, intent(in) :: E_cld_tau_w_g(:,:,:)
