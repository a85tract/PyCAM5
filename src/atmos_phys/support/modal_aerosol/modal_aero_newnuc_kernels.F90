...
!
! !INTERFACE:
module ap_modal_aero_newnuc_kernels

! !USES:
   use shr_kind_mod,  only: r8 => shr_kind_r8
   use shr_const_mod, only: pi => shr_const_pi,                 &
                            r_universal => shr_const_rgas,       &
                            avogad_kmol => shr_const_avogad
...
! !PUBLIC MEMBER FUNCTIONS:
  public :: modal_aero_newnuc_kernel

! !PUBLIC DATA MEMBERS:
! min h2so4 vapor for nuc calcs = 4.0e-16 mol/mol-air ~= 1.0e4 molecules/cm3,
  real(r8), parameter :: qh2so4_cutoff = 4.0e-16_r8
  real(r8), parameter :: rgas = r_universal*1.0e-3_r8
  real(r8), parameter :: avogad = avogad_kmol*1.0e-3_r8
...
!! \htmlinclude modal_aero_newnuc_sub_run.html
   subroutine modal_aero_newnuc_kernel(                          &
                        lchnk,    ncol,     nstep,               &
                        pcols_in, pver_in, pcnstxx_in, pcnst_in, &
                        top_lev_in, loffset, deltat,              &
                        l_h2so4_sv_in, l_nh3_sv_in,              &
                        lnumait_sv_in, lnh4ait_sv_in,            &
                        lso4ait_sv_in, lptr_nh4_aitken_in,       &
                        dgnumlo_aitken_in, dgnum_aitken_in,      &
                        dgnumhi_aitken_in, specdens_so4_in,      &
                        specmw_so4_in, specmw_nh4_in,            &
                        gravity_in,                              &
                        t,        pmid,     pdel,                &
                        zm,       pblh,                          &
                        qv,       qv_sat,   cld,                 &
...
! !PARAMETERS:
   ...
   real(r8), intent(in) :: dgnumlo_aitken_in, dgnum_aitken_in
   real(r8), intent(in) :: dgnumhi_aitken_in, specdens_so4_in
   real(r8), intent(in) :: specmw_so4_in, specmw_nh4_in
   ...
   real(r8), intent(in) :: zm(pcols_in,pver_in)   ! midpoint height above surface (m)
   real(r8), intent(in) :: pblh(pcols_in)      ! pbl height (m)
   real(r8), intent(in) :: qv(pcols_in,pver_in)   ! specific humidity (kg/kg)
   real(r8), intent(in) :: qv_sat(pcols_in,pver_in) ! saturation specific humidity
   real(r8), intent(in) :: cld(ncol,pver_in)   ! stratiform cloud fraction
                                            ! *** NOTE ncol dimension
...
!       integer, parameter :: newnuc_method_flagaa = 12
        ...
        real(r8) :: qsrflx(pcols_in,pcnst_in,nsrflx)
                              ! process-specific column tracer tendencies
                              ! 1 = nucleation (for aerocom)
        real(r8) :: dqdt(ncol,pver_in,pcnstxx_in)  ! TMR tendency array -- NOTE dims
        logical  :: dotend(pcnst_in)               ! flag for doing tendency
...
!-----------------------------------------------------------------------
        l_h2so4 = l_h2so4_sv_in - loffset
        l_nh3   = l_nh3_sv_in   - loffset
        lnumait = lnumait_sv_in - loffset
        lnh4ait = lnh4ait_sv_in - loffset
        lso4ait = lso4ait_sv_in - loffset
...
!   set dotend
        ...
        lnh4ait = lptr_nh4_aitken_in - loffset
        if ((l_nh3   > 0) .and. (l_nh3   <= pcnst_in) .and. &
            (lnh4ait > 0) .and. (lnh4ait <= pcnst_in)) then
            do_nh3 = .true.
            dotend(lnh4ait) = .true.
            dotend(l_nh3) = .true.
        else
...
!   dry-diameter limits for "grown" new particles
        dplom_mode(1) = exp( 0.67_r8*log(dgnumlo_aitken_in)   &
                           + 0.33_r8*log(dgnum_aitken_in) )
        dphim_mode(1) = dgnumhi_aitken_in
...
!        mass1p_aithi - dp = dphim_mode(1)
        tmpa = specdens_so4_in*pi/6.0_r8
...
!   fraction of mass nuc going to so4
        tmpa = qso4a_del*specmw_so4_in
        tmpb = tmpa + qnh4a_del*specmw_nh4_in
...
!   dso4dt_ait, dnh4dt_ait are (kmol/kmol-air/s)
        dso4dt_ait = dmdt_ait*tmp_frso4/specmw_so4_in
        dnh4dt_ait = dmdt_ait*(1.0_r8 - tmp_frso4)/specmw_nh4_in
...
!   do history file column-tendency fields
        do l = loffset+1, pcnst_in
            ...
            do i = 1, ncol
                qsrflx_out(i,lmz) = qsrflx(i,lmz,1)
            end do
...
!                'modal_aero_newnuc_sub outfld', fieldname, adv_mass(lmz)
        ...
        return
!EOC
        end subroutine modal_aero_newnuc_kernel
...
! HOST-FACADE-MOVED: !----------------------------------------------------------------------
! HOST-FACADE-MOVED: !----------------------------------------------------------------------
! HOST-FACADE-MOVED: subroutine modal_aero_newnuc_init
! HOST-FACADE-MOVED: 
! HOST-FACADE-MOVED: !-----------------------------------------------------------------------
! HOST-FACADE-MOVED: !
! HOST-FACADE-MOVED: ! Purpose:
! HOST-FACADE-MOVED: !    set do_adjust and do_aitken flags
! HOST-FACADE-MOVED: !    create history fields for column tendencies associated with
! HOST-FACADE-MOVED: !       modal_aero_calcsize
! HOST-FACADE-MOVED: !
! HOST-FACADE-MOVED: ! Author: R. Easter
! HOST-FACADE-MOVED: !
! HOST-FACADE-MOVED: !-----------------------------------------------------------------------
! HOST-FACADE-MOVED: 
! HOST-FACADE-MOVED: use modal_aero_data
! HOST-FACADE-MOVED: use modal_aero_rename
! HOST-FACADE-MOVED: 
...
! HOST-FACADE-MOVED: use constituents,     only:  pcnst, cnst_get_ind, cnst_name
...
! HOST-FACADE-MOVED: 
! HOST-FACADE-MOVED: !-----------------------------------------------------------------------
! HOST-FACADE-MOVED: ! arguments
! HOST-FACADE-MOVED: 
! HOST-FACADE-MOVED: !-----------------------------------------------------------------------
! HOST-FACADE-MOVED: ! local
! HOST-FACADE-MOVED:    integer  :: l_h2so4, l_nh3
! HOST-FACADE-MOVED:    integer  :: lnumait, lnh4ait, lso4ait
! HOST-FACADE-MOVED:    integer  :: l
! HOST-FACADE-MOVED:    integer  :: m, mait
! HOST-FACADE-MOVED: 
...
! HOST-FACADE-MOVED: 
! HOST-FACADE-MOVED:    logical                        :: dotend(pcnst)
...
! HOST-FACADE-MOVED: 
! HOST-FACADE-MOVED: 
! HOST-FACADE-MOVED: !   set these indices
! HOST-FACADE-MOVED: !   skip if no h2so4 species
! HOST-FACADE-MOVED: !   skip if no aitken mode so4 or num species
! HOST-FACADE-MOVED:         l_h2so4_sv = 0
! HOST-FACADE-MOVED:         l_nh3_sv = 0
! HOST-FACADE-MOVED:         lnumait_sv = 0
! HOST-FACADE-MOVED:         lnh4ait_sv = 0
! HOST-FACADE-MOVED:         lso4ait_sv = 0
! HOST-FACADE-MOVED: 
! HOST-FACADE-MOVED:         call cnst_get_ind( 'H2SO4', l_h2so4, .false. )
! HOST-FACADE-MOVED:         call cnst_get_ind( 'NH3', l_nh3, .false. )
! HOST-FACADE-MOVED: 
! HOST-FACADE-MOVED:         mait = modeptr_aitken
! HOST-FACADE-MOVED:         if (mait > 0) then
! HOST-FACADE-MOVED:             lnumait = numptr_amode(mait)
! HOST-FACADE-MOVED:             lso4ait = lptr_so4_a_amode(mait)
! HOST-FACADE-MOVED:             lnh4ait = lptr_nh4_a_amode(mait)
! HOST-FACADE-MOVED:         end if
! HOST-FACADE-MOVED:         if ((l_h2so4  <= 0) .or. (l_h2so4 > pcnst)) then
! HOST-FACADE-MOVED:             write(*,'(/a/)')   &
! HOST-FACADE-MOVED:                 '*** modal_aero_newnuc bypass -- l_h2so4 <= 0'
! HOST-FACADE-MOVED:             return
! HOST-FACADE-MOVED:         else if ((lso4ait <= 0) .or. (lso4ait > pcnst)) then
! HOST-FACADE-MOVED:             write(*,'(/a/)')   &
! HOST-FACADE-MOVED:                 '*** modal_aero_newnuc bypass -- lso4ait <= 0'
! HOST-FACADE-MOVED:             return
! HOST-FACADE-MOVED:         else if ((lnumait <= 0) .or. (lnumait > pcnst)) then
! HOST-FACADE-MOVED:             write(*,'(/a/)')   &
! HOST-FACADE-MOVED:                 '*** modal_aero_newnuc bypass -- lnumait <= 0'
! HOST-FACADE-MOVED:             return
! HOST-FACADE-MOVED:         else if ((mait <= 0) .or. (mait > ntot_amode)) then
! HOST-FACADE-MOVED:             write(*,'(/a/)')   &
! HOST-FACADE-MOVED:                 '*** modal_aero_newnuc bypass -- modeptr_aitken <= 0'
! HOST-FACADE-MOVED:             return
! HOST-FACADE-MOVED:         end if
! HOST-FACADE-MOVED: 
! HOST-FACADE-MOVED:         l_h2so4_sv = l_h2so4
! HOST-FACADE-MOVED:         l_nh3_sv   = l_nh3
! HOST-FACADE-MOVED:         lnumait_sv = lnumait
! HOST-FACADE-MOVED:         lnh4ait_sv = lnh4ait
! HOST-FACADE-MOVED:         lso4ait_sv = lso4ait
! HOST-FACADE-MOVED: 
! HOST-FACADE-MOVED: !
! HOST-FACADE-MOVED: !   create history file column-tendency fields
! HOST-FACADE-MOVED: !
! HOST-FACADE-MOVED:         dotend(:) = .false.
! HOST-FACADE-MOVED:         dotend(lnumait) = .true.
! HOST-FACADE-MOVED:         dotend(lso4ait) = .true.
! HOST-FACADE-MOVED:         dotend(l_h2so4) = .true.
! HOST-FACADE-MOVED:         if ((l_nh3   > 0) .and. (l_nh3   <= pcnst) .and. &
! HOST-FACADE-MOVED:             (lnh4ait > 0) .and. (lnh4ait <= pcnst)) then
! HOST-FACADE-MOVED:             dotend(lnh4ait) = .true.
! HOST-FACADE-MOVED:             dotend(l_nh3) = .true.
! HOST-FACADE-MOVED:         end if
! HOST-FACADE-MOVED: 
! HOST-FACADE-MOVED:         do l = 1, pcnst
! HOST-FACADE-MOVED:             if ( .not. dotend(l) ) cycle
! HOST-FACADE-MOVED:             tmpname = cnst_name(l)
! HOST-FACADE-MOVED:             unit = 'kg/m2/s'
! HOST-FACADE-MOVED:             do m = 1, ntot_amode
! HOST-FACADE-MOVED:                 if (l == numptr_amode(m)) unit = '#/m2/s'
! HOST-FACADE-MOVED:             end do
! HOST-FACADE-MOVED:             fieldname = trim(tmpname) // '_sfnnuc1'
! HOST-FACADE-MOVED:             long_name = trim(tmpname) // ' modal_aero new particle nucleation column tendency'
...
! HOST-FACADE-MOVED:                 'modal_aero_newnuc_init addfld', fieldname, unit
! HOST-FACADE-MOVED:         end do ! l = ...
! HOST-FACADE-MOVED: 
! HOST-FACADE-MOVED: 
! HOST-FACADE-MOVED:       return
! HOST-FACADE-MOVED:       end subroutine modal_aero_newnuc_init
! HOST-FACADE-MOVED: 
! HOST-FACADE-MOVED: 
! HOST-FACADE-MOVED: 
! HOST-FACADE-MOVED: !----------------------------------------------------------------------
...
!----------------------------------------------------------------------
end module ap_modal_aero_newnuc_kernels
