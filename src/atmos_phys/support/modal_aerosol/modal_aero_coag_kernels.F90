...
!
! !INTERFACE:
   module ap_modal_aero_coag_kernels
...
!! \htmlinclude modal_aero_coag_sub_run.html
   subroutine modal_aero_coag_kernel(                            &
                        lchnk,    ncol,     nstep,               &
                        ...
                        maxpair_in,                             &
                        loffset,  deltat_main,                   &
                        pair_option_in, npair_in,                &
                        modefrm_in, modetoo_in, nspecfrm_in,     &
                        lspecfrm_in, lspectoo_in,                &
                        ...
                        n_so4_monolayers_pcage_in,               &
                        soa_equivso4_factor_in,                  &
                        tmelt_in,                                &
                        t,        pmid,     pdel,                &
                        q,                                       &
                        dgncur_a,           dgncur_awet,         &
                        wetdens_a, qsrflx_out, dotend_out,       &
                        ierr_out                                )
...
! !PARAMETERS:
   ...
   integer, intent(in)  :: pcnst_in         ! host constituent dimension
   integer, intent(in)  :: top_lev_in       ! top active modal-aerosol level
   integer, intent(in)  :: maxspec_in       ! maximum aerosol species dimension
   integer, intent(in)  :: ntot_aspectype_in ! active aerosol species types
   integer, intent(in)  :: maxpair_in       ! maximum coagulation pair dimension
   ...
   real(r8), intent(in) :: specmw_so4_amode_in, specdens_so4_amode_in
   real(r8), intent(in) :: specmw_nh4_amode_in, specdens_nh4_amode_in
   real(r8), intent(in) :: specmw_soa_amode_in, specdens_soa_amode_in
   real(r8), intent(in) :: n_so4_monolayers_pcage_in
   real(r8), intent(in) :: soa_equivso4_factor_in
   real(r8), intent(in) :: tmelt_in

   real(r8), intent(in) :: t(pcols_in,pver_in)    ! temperature (K)
...
!        aitken mode (modeptr_aitken_in) with accumulation mode (modeptr_accum_in)
!   this version will
...
! local variables
        ...
        integer :: idomode(ntot_amode_in), iselfcoagdone(ntot_amode_in)
        integer :: jfreqcoag
        integer :: k
        ...
        real(r8) :: dryvol_a(pcols_in,pver_in,ntot_amode_in)
              real(r8) :: dumexp, dumloss, dumprod
        real(r8) :: dumsfc_frm_old, dumsfc_frm_new
        real(r8) :: dum_m2v
        real(r8) :: fac_m2v_aitage(maxspec_in), fac_m2v_pcarbon(maxspec_in)
        ...
        real(r8) :: xnumbconc(ntot_amode_in)
        real(r8) :: xnumbconcavg(ntot_amode_in), xnumbconcnew(ntot_amode_in)
        real(r8) :: ybetaij0(maxpair_in), ybetaij3(maxpair_in)
        real(r8) :: ybetaii0(maxpair_in), ybetajj0(maxpair_in)

        real(r8) :: dqdt(ncol,pver_in,pcnstxx_in)  ! TMR "dq/dt" array - NOTE dims
        logical  :: dotend(pcnst_in)            ! identifies the species that
                                             ! tendencies are computed for
        real(r8) :: qsrflx(pcols_in)
...
!!$         '*** modal_aero_coag_sub -- nstep, iam, lat, lon, pcols_in, ncol =',   &
!!$         nstep, iam, latndx(i), lonndx(i), pcols_in, ncol
!!$   end do
!!$   end if
...
!
        idomode(:) = 0
        do ipair = 1, npair_in
            idomode(modefrm_in(ipair)) = 1
            idomode(modetoo_in(ipair)) = 1
        end do

!
!   other init
!
        macc = modeptr_accum_in
        mait = modeptr_aitken_in
        mpca = modeptr_pcarbon_in

        fac_m2v_aitage(:) = 0.0_r8
        fac_m2v_pcarbon(:) = 0.0_r8
        if (pair_option_in == 3) then
!   following ipair definitions MUST BE CONSISTENT with
!   the coding in modal_aero_coag_init for pair_option_in == 3
            ip_aitacc = 1
            ip_pcaacc = 2
            ip_aitpca = 3

        ! use 1 mol (bi-)sulfate = 65 cm^3 --> 1 molecule = (4.76e-10 m)^3
            dr_so4_monolayers_pcage = n_so4_monolayers_pcage_in * 4.76e-10_r8

            ipair = ip_aitpca
            do iq = 1, nspecfrm_in(ipair)
                lsfrm = lspecfrm_in(iq,ipair)
                if (lsfrm == lptr_so4_a_amode_in(mait)) then
                    fac_m2v_aitage(iq) = specmw_so4_amode_in / specdens_so4_amode_in
                else if (lsfrm == lptr_nh4_a_amode_in(mait)) then
                    fac_m2v_aitage(iq) = specmw_nh4_amode_in / specdens_nh4_amode_in
                else if (lsfrm == lptr_soa_a_amode_in(mait)) then
                    fac_m2v_aitage(iq) = soa_equivso4_factor_in*   &
                                        (specmw_soa_amode_in / specdens_soa_amode_in)
!   for soa, the soa_equivso4_factor_in converts the soa volume into an
...
!   (see modal_aero_gasaerexch)
            ...
            do l = 1, nspec_amode_in(mpca)
                l2 = lspectype_amode_in(l,mpca)
...
!                [m3-AP/kmol-AP]    = [kg-AP/kmol-AP]  / [kg-AP/m3-AP]
            ...
            fac_volsfc_pcarbon = exp( 2.5_r8*(alnsg_amode_in(mpca)**2) )
        else
            ip_aitacc = -999888777
            ip_pcaacc = -999888777
            ip_aitpca = -999888777
...
main_k: do k = top_lev_in, pver_in
main_i: do i = 1, ncol
...
!   calculate number conc. (#/m3) for modes doing coagulation
        do n = 1, ntot_amode_in
            if (idomode(n) .gt. 0) then
                xnumbconc(n) = q(i,k,numptr_amode_in(n)-loffset)*aircon
...
!
main_ipair1: do ipair = 1, npair_in

        modefrm = modefrm_in(ipair)
        modetoo = modetoo_in(ipair)
...
!
        call getcoags_wrapper_f(                                       &
          t(i,k), pmid(i,k), tmelt_in,                                &
          dgncur_awet(i,k,modefrm),     dgncur_awet(i,k,modetoo),      &
          sigmag_amode_in(modefrm),        sigmag_amode_in(modetoo),         &
          alnsg_amode_in(modefrm),         alnsg_amode_in(modetoo),          &
...
!!$        sg_frm   = sigmag_amode_in(modefrm)
!!$        sg_too   = sigmag_amode_in(modetoo)
!!$        lnsg_frm = alnsg_amode_in(modefrm)
!!$        lnsg_too = alnsg_amode_in(modetoo)
!!$
!!$        call getcoags_wrapper_f(                                       &
!!$          t(i,k), pmid(i,k),                                           &
...
!   test diagnostics end ----------------------------------------------
        ...
        if ( (pair_option_in == 1) .or.   &
             (pair_option_in == 2) ) then
!
!   calculate number and mass changes for pair_option_in == 1,2
!
main_ipair2: do ipair = 1, npair_in
...
!        next  calc change to "frm" mode, using average number conc of "too"
        if ( (mprognum_amode_in(modetoo) > 0) .and.   &
        ...
        if ( (mprognum_amode_in(modefrm) > 0) .and.   &
            ...
            lsfrm = numptr_amode_in(modefrm) - loffset
...
!     xferfracvol = fraction of modefrm volume transferred to modetoo over deltat
        ...
        do iq = 1, nspecfrm_in(ipair)
            lsfrm = lspecfrm_in(iq,ipair) - loffset
            lstoo = lspectoo_in(iq,ipair) - loffset
            if (lsfrm > 0) then
        ...
        else if (pair_option_in == 3) then
!
!   calculate number and mass changes for pair_option_in == 3
!

!   calculate number changes to accum mode
        if (mprognum_amode_in(macc) > 0) then
            tmpn = xnumbconc(macc)
            xnumbconcnew(macc) = tmpn/(1.0_r8 + deltat*ybetajj0(ip_aitacc)*tmpn)
            xnumbconcavg(macc) = 0.5_r8*(xnumbconcnew(macc) + tmpn)
            lstoo = numptr_amode_in(macc) - loffset
...
!   calculate number changes to primary carbon mode
        modefrm = modeptr_pcarbon_in
        if (mprognum_amode_in(mpca) > 0) then
            tmpn = xnumbconc(mpca)
            ...
            lsfrm = numptr_amode_in(mpca) - loffset
...
!   calculate number changes to aitken mode
        if (mprognum_amode_in(mait) > 0) then
            tmpn = xnumbconc(mait)
            ...
            lsfrm = numptr_amode_in(mait) - loffset
...
!   this duplicates the code in modal_aero_gasaerexch
        ...
        do l = 1, nspec_amode_in(mpca)
            vol_core = vol_core + &
                q(i,k,lmassptr_amode_in(l,mpca)-loffset)*fac_m2v_pcarbon(l)
        end do
...
!   and aging
        ...
        else   ! (pair_option_in /= 1,2,3) then

        ierr_out = 1
        return


        end if   ! (pair_option_in == ...)
...
! set dotend's
        do ipair = 1, npair_in
            ...
            if (mprognum_amode_in(modefrm) > 0) then
                lsfrm = numptr_amode_in(modefrm) - loffset
                if (lsfrm > 0) dotend(lsfrm) = .true.
            end if
            if (mprognum_amode_in(modetoo) > 0) then
                lstoo = numptr_amode_in(modetoo) - loffset
                if (lstoo > 0) dotend(lstoo) = .true.
            end if
...
!EOC
        end subroutine modal_aero_coag_kernel


! HOST-FACADE-MOVED: !----------------------------------------------------------------------
! HOST-FACADE-MOVED: !----------------------------------------------------------------------
! HOST-FACADE-MOVED:         subroutine modal_aero_coag_init
! HOST-FACADE-MOVED: !
! HOST-FACADE-MOVED: !   computes pointers for species transfer during coagulation
! HOST-FACADE-MOVED: !
! HOST-FACADE-MOVED:         use modal_aero_data
! HOST-FACADE-MOVED:         use modal_aero_gasaerexch, only:  &
! HOST-FACADE-MOVED:                 modefrm_pcage, nspecfrm_pcage, lspecfrm_pcage, lspectoo_pcage
! HOST-FACADE-MOVED: 
! HOST-FACADE-MOVED:         use cam_abortutils,  only: endrun
! HOST-FACADE-MOVED:         use cam_history,     only: addfld, add_default, fieldname_len, phys_decomp
! HOST-FACADE-MOVED:         use constituents,    only: pcnst_in, cnst_name
! HOST-FACADE-MOVED:         use spmd_utils,      only: masterproc
! HOST-FACADE-MOVED:         use phys_control,    only: phys_getopts
! HOST-FACADE-MOVED: 
! HOST-FACADE-MOVED:         implicit none
! HOST-FACADE-MOVED: 
! HOST-FACADE-MOVED: !   local variables
! HOST-FACADE-MOVED:         integer :: ipair, iq, iqfrm, iqfrm_aa, iqtoo, iqtoo_aa
! HOST-FACADE-MOVED:         integer :: l, lsfrm, lstoo, lunout
! HOST-FACADE-MOVED:         integer :: m, mfrm, mtoo, mtef
! HOST-FACADE-MOVED:         integer :: nsamefrm, nsametoo, nspec
! HOST-FACADE-MOVED: 
! HOST-FACADE-MOVED:         character(len=fieldname_len)   :: tmpname
! HOST-FACADE-MOVED:         character(len=fieldname_len+3) :: fieldname
! HOST-FACADE-MOVED:         character(128)                 :: long_name
! HOST-FACADE-MOVED:         character(8)                   :: unit
! HOST-FACADE-MOVED: 
! HOST-FACADE-MOVED:         logical :: dotend(pcnst_in)
! HOST-FACADE-MOVED:         logical :: history_aerosol      ! Output the MAM aerosol tendencies
! HOST-FACADE-MOVED: 
! HOST-FACADE-MOVED:         !-----------------------------------------------------------------------
! HOST-FACADE-MOVED: 
! HOST-FACADE-MOVED:         call phys_getopts( history_aerosol_out        = history_aerosol   )
! HOST-FACADE-MOVED: 
! HOST-FACADE-MOVED:         lunout = 6
! HOST-FACADE-MOVED: !
! HOST-FACADE-MOVED: !   define "from mode" and "to mode" for each coagulation pairing
! HOST-FACADE-MOVED: !        currently just a2-->a1 coagulation
! HOST-FACADE-MOVED: !
! HOST-FACADE-MOVED:         if (pair_option_in == 1) then
! HOST-FACADE-MOVED:             npair_in = 1
! HOST-FACADE-MOVED:             modefrm_in(1) = modeptr_aitken_in
! HOST-FACADE-MOVED:             modetoo_in(1) = modeptr_accum_in
! HOST-FACADE-MOVED:             modetooeff_in(1) = modeptr_accum_in
! HOST-FACADE-MOVED:         else if (pair_option_in == 2) then
! HOST-FACADE-MOVED:             npair_in = 2
! HOST-FACADE-MOVED:             modefrm_in(1) = modeptr_aitken_in
! HOST-FACADE-MOVED:             modetoo_in(1) = modeptr_accum_in
! HOST-FACADE-MOVED:             modetooeff_in(1) = modeptr_accum_in
! HOST-FACADE-MOVED:             modefrm_in(2) = modeptr_pcarbon_in
! HOST-FACADE-MOVED:             modetoo_in(2) = modeptr_accum_in
! HOST-FACADE-MOVED:             modetooeff_in(2) = modeptr_accum_in
! HOST-FACADE-MOVED:         else if (pair_option_in == 3) then
! HOST-FACADE-MOVED:             npair_in = 3
! HOST-FACADE-MOVED:             modefrm_in(1) = modeptr_aitken_in
! HOST-FACADE-MOVED:             modetoo_in(1) = modeptr_accum_in
! HOST-FACADE-MOVED:             modetooeff_in(1) = modeptr_accum_in
! HOST-FACADE-MOVED:             modefrm_in(2) = modeptr_pcarbon_in
! HOST-FACADE-MOVED:             modetoo_in(2) = modeptr_accum_in
! HOST-FACADE-MOVED:             modetooeff_in(2) = modeptr_accum_in
! HOST-FACADE-MOVED:             modefrm_in(3) = modeptr_aitken_in
! HOST-FACADE-MOVED:             modetoo_in(3) = modeptr_pcarbon_in
! HOST-FACADE-MOVED:             modetooeff_in(3) = modeptr_accum_in
! HOST-FACADE-MOVED:             if (modefrm_pcage <= 0) then
! HOST-FACADE-MOVED:                 write(*,*) '*** modal_aero_coag_init error'
! HOST-FACADE-MOVED:                 write(*,*) '    pair_option_in, modefrm_pcage mismatch'
! HOST-FACADE-MOVED:                 write(*,*) '    pair_option_in, modefrm_pcage =', &
! HOST-FACADE-MOVED:                     pair_option_in, modefrm_pcage
! HOST-FACADE-MOVED:                 call endrun( 'modal_aero_coag_init error' )
! HOST-FACADE-MOVED:             end if
! HOST-FACADE-MOVED:         else
! HOST-FACADE-MOVED:             npair_in = 0
! HOST-FACADE-MOVED:             return
! HOST-FACADE-MOVED:         end if
! HOST-FACADE-MOVED: 
! HOST-FACADE-MOVED: !
! HOST-FACADE-MOVED: !   define species involved in each coagulation pairing
! HOST-FACADE-MOVED: !        (include aerosol water)
! HOST-FACADE-MOVED: !
! HOST-FACADE-MOVED: aa_ipair: do ipair = 1, npair_in
! HOST-FACADE-MOVED: 
! HOST-FACADE-MOVED:         mfrm = modefrm_in(ipair)
! HOST-FACADE-MOVED:         mtoo = modetoo_in(ipair)
! HOST-FACADE-MOVED:         mtef = modetooeff_in(ipair)
! HOST-FACADE-MOVED:         if ( (mfrm < 1) .or. (mfrm > ntot_amode_in) .or.   &
! HOST-FACADE-MOVED:              (mtoo < 1) .or. (mtoo > ntot_amode_in) .or.   &
! HOST-FACADE-MOVED:              (mtef < 1) .or. (mtef > ntot_amode_in) ) then
! HOST-FACADE-MOVED:             write(*,*) '*** modal_aero_coag_init error'
! HOST-FACADE-MOVED:             write(*,*) '    ipair, ntot_amode_in =', ipair, ntot_amode_in
! HOST-FACADE-MOVED:             write(*,*) '    mfrm, mtoo, mtef  =', mfrm, mtoo, mtef
! HOST-FACADE-MOVED:             call endrun( 'modal_aero_coag_init error' )
! HOST-FACADE-MOVED:         end if
! HOST-FACADE-MOVED: 
! HOST-FACADE-MOVED: 
! HOST-FACADE-MOVED:         mtoo = mtef   ! effective modetoo
! HOST-FACADE-MOVED:         nspec = 0
! HOST-FACADE-MOVED: aa_iqfrm: do iqfrm = 1, nspec_amode_in(mfrm)
! HOST-FACADE-MOVED:             lsfrm = lmassptr_amode_in(iqfrm,mfrm)
! HOST-FACADE-MOVED:             if ((lsfrm .lt. 1) .or. (lsfrm .gt. pcnst_in)) cycle aa_iqfrm
! HOST-FACADE-MOVED: 
! HOST-FACADE-MOVED: ! find "too" species having same lspectype_amode_in as the "frm" species
! HOST-FACADE-MOVED: ! several species in a mode may have the same lspectype_amode_in, so also
! HOST-FACADE-MOVED: !    use the ordering as a criterion (e.g., 1st <--> 1st, 2nd <--> 2nd)
! HOST-FACADE-MOVED:             iqfrm_aa = 1
! HOST-FACADE-MOVED:             iqtoo_aa = 1
! HOST-FACADE-MOVED:             if (iqfrm .gt. nspec_amode_in(mfrm)) then
! HOST-FACADE-MOVED:                 iqfrm_aa = nspec_amode_in(mfrm) + 1
! HOST-FACADE-MOVED:                 iqtoo_aa = nspec_amode_in(mtoo) + 1
! HOST-FACADE-MOVED:             end if
! HOST-FACADE-MOVED:             nsamefrm = 0
! HOST-FACADE-MOVED:             do iq = iqfrm_aa, iqfrm
! HOST-FACADE-MOVED:                 if ( lspectype_amode_in(iq   ,mfrm) .eq.   &
! HOST-FACADE-MOVED:                            lspectype_amode_in(iqfrm,mfrm) ) then
! HOST-FACADE-MOVED:                     nsamefrm = nsamefrm + 1
! HOST-FACADE-MOVED:                 end if
! HOST-FACADE-MOVED:             end do
! HOST-FACADE-MOVED:             nsametoo = 0
! HOST-FACADE-MOVED:             lstoo = 0
! HOST-FACADE-MOVED:             do iqtoo = iqtoo_aa, nspec_amode_in(mtoo)
! HOST-FACADE-MOVED:                 if ( lspectype_amode_in(iqtoo,mtoo) .eq.   &
! HOST-FACADE-MOVED:                            lspectype_amode_in(iqfrm,mfrm) ) then
! HOST-FACADE-MOVED:                     nsametoo = nsametoo + 1
! HOST-FACADE-MOVED:                     if (nsametoo .eq. nsamefrm) then
! HOST-FACADE-MOVED:                         lstoo = lmassptr_amode_in(iqtoo,mtoo)
! HOST-FACADE-MOVED:                         exit
! HOST-FACADE-MOVED:                     end if
! HOST-FACADE-MOVED:                 end if
! HOST-FACADE-MOVED:             end do
! HOST-FACADE-MOVED: 
! HOST-FACADE-MOVED:             nspec = nspec + 1
! HOST-FACADE-MOVED:             lspecfrm_in(nspec,ipair) = lsfrm
! HOST-FACADE-MOVED:             lspectoo_in(nspec,ipair) = lstoo
! HOST-FACADE-MOVED:         end do aa_iqfrm
! HOST-FACADE-MOVED: 
! HOST-FACADE-MOVED: !       lsfrm = lwaterptr_amode(mfrm)
! HOST-FACADE-MOVED: !       if ((lsfrm .ge. 1) .and. (lsfrm .le. pcnst_in)) then
! HOST-FACADE-MOVED: !           lstoo = lwaterptr_amode(mtoo)
! HOST-FACADE-MOVED: !           if ((lstoo .lt. 1) .or. (lstoo .gt. pcnst_in)) lstoo = 0
! HOST-FACADE-MOVED: !           nspec = nspec + 1
! HOST-FACADE-MOVED: !           lspecfrm_in(nspec,ipair) = lsfrm
! HOST-FACADE-MOVED: !           lspectoo_in(nspec,ipair) = lstoo
! HOST-FACADE-MOVED: !       end if
! HOST-FACADE-MOVED: 
! HOST-FACADE-MOVED:         nspecfrm_in(ipair) = nspec
! HOST-FACADE-MOVED:         end do aa_ipair
! HOST-FACADE-MOVED: 
! HOST-FACADE-MOVED: !
! HOST-FACADE-MOVED: !   output results
! HOST-FACADE-MOVED: !
! HOST-FACADE-MOVED:         if ( masterproc ) then
! HOST-FACADE-MOVED: 
! HOST-FACADE-MOVED:         write(lunout,9310)
! HOST-FACADE-MOVED: 
! HOST-FACADE-MOVED:         do ipair = 1, npair_in
! HOST-FACADE-MOVED:           mfrm = modefrm_in(ipair)
! HOST-FACADE-MOVED:           mtoo = modetoo_in(ipair)
! HOST-FACADE-MOVED:           mtef = modetooeff_in(ipair)
! HOST-FACADE-MOVED:           write(lunout,9320) ipair, mfrm, mtoo, mtef
! HOST-FACADE-MOVED: 
! HOST-FACADE-MOVED:           do iq = 1, nspecfrm_in(ipair)
! HOST-FACADE-MOVED:             lsfrm = lspecfrm_in(iq,ipair)
! HOST-FACADE-MOVED:             lstoo = lspectoo_in(iq,ipair)
! HOST-FACADE-MOVED:             if (lstoo .gt. 0) then
! HOST-FACADE-MOVED:                 write(lunout,9330) lsfrm, cnst_name(lsfrm),   &
! HOST-FACADE-MOVED:                               lstoo, cnst_name(lstoo)
! HOST-FACADE-MOVED:             else
! HOST-FACADE-MOVED:                 write(lunout,9340) lsfrm, cnst_name(lsfrm)
! HOST-FACADE-MOVED:             end if
! HOST-FACADE-MOVED:           end do
! HOST-FACADE-MOVED: 
! HOST-FACADE-MOVED:         end do ! ipair = ...
! HOST-FACADE-MOVED:         write(lunout,*)
! HOST-FACADE-MOVED: 
! HOST-FACADE-MOVED:         end if ! ( masterproc )
! HOST-FACADE-MOVED: 
! HOST-FACADE-MOVED: 9310        format( / 'subr. modal_aero_coag_init' )
! HOST-FACADE-MOVED: 9320        format( 'pair', i3, 5x, 'mode', i3, &
! HOST-FACADE-MOVED:                 ' ---> mode', i3, '   eff', i3 )
! HOST-FACADE-MOVED: 9330        format( 5x, 'spec', i3, '=', a, ' ---> spec', i3, '=', a )
! HOST-FACADE-MOVED: 9340        format( 5x, 'spec', i3, '=', a, ' ---> LOSS' )
! HOST-FACADE-MOVED: 
! HOST-FACADE-MOVED: 
! HOST-FACADE-MOVED: !
! HOST-FACADE-MOVED: !   create history file column-tendency fields
! HOST-FACADE-MOVED: !
! HOST-FACADE-MOVED:         dotend(:) = .false.
! HOST-FACADE-MOVED:         do ipair = 1, npair_in
! HOST-FACADE-MOVED:           do iq = 1, nspecfrm_in(ipair)
! HOST-FACADE-MOVED:             l = lspecfrm_in(iq,ipair)
! HOST-FACADE-MOVED:             if ((l > 0) .and. (l <= pcnst_in)) dotend(l) = .true.
! HOST-FACADE-MOVED:             l = lspectoo_in(iq,ipair)
! HOST-FACADE-MOVED:             if ((l > 0) .and. (l <= pcnst_in)) dotend(l) = .true.
! HOST-FACADE-MOVED:           end do
! HOST-FACADE-MOVED: 
! HOST-FACADE-MOVED:           m = modefrm_in(ipair)
! HOST-FACADE-MOVED:           if ((m > 0) .and. (m <= ntot_amode_in)) then
! HOST-FACADE-MOVED:             l = numptr_amode_in(m)
! HOST-FACADE-MOVED:             if ((l > 0) .and. (l <= pcnst_in)) dotend(l) = .true.
! HOST-FACADE-MOVED:           end if
! HOST-FACADE-MOVED:           m = modetoo_in(ipair)
! HOST-FACADE-MOVED:           if ((m > 0) .and. (m <= ntot_amode_in)) then
! HOST-FACADE-MOVED:             l = numptr_amode_in(m)
! HOST-FACADE-MOVED:             if ((l > 0) .and. (l <= pcnst_in)) dotend(l) = .true.
! HOST-FACADE-MOVED:           end if
! HOST-FACADE-MOVED:         end do ! ipair = ...
! HOST-FACADE-MOVED: 
! HOST-FACADE-MOVED:         if (pair_option_in == 3) then
! HOST-FACADE-MOVED:            do iq = 1, nspecfrm_pcage
! HOST-FACADE-MOVED:               lsfrm = lspecfrm_pcage(iq)
! HOST-FACADE-MOVED:               lstoo = lspectoo_pcage(iq)
! HOST-FACADE-MOVED:               if ((lsfrm > 0) .and. (lsfrm <= pcnst_in)) then
! HOST-FACADE-MOVED:                  dotend(lsfrm) = .true.
! HOST-FACADE-MOVED:                  if ((lstoo > 0) .and. (lstoo <= pcnst_in)) then
! HOST-FACADE-MOVED:                     dotend(lstoo) = .true.
! HOST-FACADE-MOVED:                  end if
! HOST-FACADE-MOVED:               end if
! HOST-FACADE-MOVED:            end do
! HOST-FACADE-MOVED:         end if
! HOST-FACADE-MOVED: 
! HOST-FACADE-MOVED:         do l = 1, pcnst_in
! HOST-FACADE-MOVED:             if ( .not. dotend(l) ) cycle
! HOST-FACADE-MOVED:             tmpname = cnst_name(l)
! HOST-FACADE-MOVED:             unit = 'kg/m2/s'
! HOST-FACADE-MOVED:             do m = 1, ntot_amode_in
! HOST-FACADE-MOVED:                 if (l == numptr_amode_in(m)) unit = '#/m2/s'
! HOST-FACADE-MOVED:             end do
! HOST-FACADE-MOVED:             fieldname = trim(tmpname) // '_sfcoag1'
! HOST-FACADE-MOVED:             long_name = trim(tmpname) // ' modal_aero coagulation column tendency'
! HOST-FACADE-MOVED:             call addfld( fieldname, unit, 1, 'A', long_name, phys_decomp )
! HOST-FACADE-MOVED:             if ( history_aerosol ) then
! HOST-FACADE-MOVED:                call add_default( fieldname, 1, ' ' )
! HOST-FACADE-MOVED:             endif
! HOST-FACADE-MOVED:             if ( masterproc ) write(*,'(3(a,2x))') &
! HOST-FACADE-MOVED:                 'modal_aero_coag_init addfld', fieldname, unit
! HOST-FACADE-MOVED:         end do ! l = ...
! HOST-FACADE-MOVED: 
! HOST-FACADE-MOVED: 
! HOST-FACADE-MOVED:         return
! HOST-FACADE-MOVED:         end subroutine modal_aero_coag_init
! HOST-FACADE-MOVED: 
! HOST-FACADE-MOVED: !----------------------------------------------------------------------
! HOST-FACADE-MOVED: !----------------------------------------------------------------------
      subroutine getcoags_wrapper_f(              &
          airtemp, airprs, tmelt_in,              &
          dgatk, dgacc,                           &
          sgatk, sgacc,                           &
          xxlsgat, xxlsgac,                       &
          pdensat, pdensac,                       &
...
! *** arguments
      ...
      real(r8), intent(in) :: tmelt_in ! water melting temperature [K]
...
!----------------------------------------------------------------------
   ...
   end module ap_modal_aero_coag_kernels
