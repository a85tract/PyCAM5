...
use wv_saturation_portable, only: qsat_water, svp_water, svp_ice
...
subroutine cldfrc2m_portable_init(rair_in, rhmini_in, rhmaxi_in, rhminl_in, &
     rhminl_adj_land_in, rhminh_in, premit_in, premib_in, iceopt_in, icecrit_in)
...
end subroutine cldfrc2m_portable_init
...
subroutine aist_vector(qv_in, T_in, p_in, qi_in, ni_in, landfrac_in, snowh_in, aist_out, ncol, &
   ...
   real(r8) ncf,phi                         ! Wilson and Ballard parameters
   real(r8) qs
   real(r8) esat_in(size(qv_in))
   real(r8) qsat_in(size(qv_in))
