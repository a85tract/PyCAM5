...
logical  :: preexisting_ice_enabled
logical  :: hetfrz_classnuc_enabled
integer  :: iulog
real(r8) :: pi
real(r8) :: mincld
...
subroutine nucleati_init( &
   preexisting_ice_in, hetfrz_classnuc_in, iulog_in, pi_in, &
   mincld_in, subgrid_in)

   logical,  intent(in) :: preexisting_ice_in
   logical,  intent(in) :: hetfrz_classnuc_in
   integer,  intent(in) :: iulog_in
   real(r8), intent(in) :: pi_in
   real(r8), intent(in) :: mincld_in
   real(r8), intent(in) :: subgrid_in

   preexisting_ice_enabled = preexisting_ice_in
   hetfrz_classnuc_enabled = hetfrz_classnuc_in
   iulog               = iulog_in
   pi                  = pi_in
   mincld              = mincld_in
   subgrid             = subgrid_in
...
subroutine nucleati(  &
   ...
   ! temp variables that depend on preexisting_ice_enabled
   wbar1 = wbar
   wbar2 = wbar
...
!   It changes answers so needs climate validation.
   ...
   if (hetfrz_classnuc_enabled) nimey = 0._r8
