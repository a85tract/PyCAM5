...
contains
  ...
  subroutine atm_export( cam_out, a2x )
   ...
   !water tracers:
    ...
    !-----------------------------------------------------------------------
    interface
       function atm_export_codon(tag) result(tag_out) bind(c, name='atm_export_codon')
         import :: c_int64_t
         integer(c_int64_t), value :: tag
         integer(c_int64_t) :: tag_out
       end function atm_export_codon
    end interface
    ...
    call cam_codon_get_impl('ATM_EXPORT_IMPL', rt_codon_impl_name, rt_codon_n, rt_codon_status)
    if (.not. (rt_codon_status == 0 .and. rt_codon_n > 0 .and. &
         trim(adjustl(rt_codon_impl_name(:rt_codon_n))) == 'native')) then
       rt_codon_tag_out = atm_export_codon(int(206, c_int64_t))
       if (rt_codon_tag_out /= int(206, c_int64_t)) then
          write(iulog,*) 'atm_export_codon tag roundtrip failed'
          stop 2
       endif
       if (.not. rt_codon_proof_seen) then
          write(iulog,*) 'atm_export implementation = codon'
